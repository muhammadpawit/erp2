      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            <h3 class="box-title">Pengiriman Barang Toko</h3>
                ...
                <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
                  <table class="table">
                    <tr>
                     ...
                     <tr>
                        <td><span class="required">*</span>Lokasi Tujuan</td>
                        <td><select name="tujuan" class="form-control">
                    			<?php
                    			foreach($tujuans as $g){
                    			?>
                    				<option value="<?php echo $g['pameran_id']; ?>" ><?php echo $g['lokasi']; ?></option>
                    			<?php
                    			}
                    			?>
                    		</select>
...
$('.sidebar-menu').find('#menu-pengiriman-barang-toko').addClass('active');
...
function addModule() {
  ...
  html +='<td class="center" style="width: 3px;"><img src="view/image/delete.png" onclick="$(\'#product-row'+product_row+'\').remove()" style="cursor: pointer;" /></td>';
	...
	html += '    <td class="left"><input type="text" data-id="'+product_row+'" onkeyup="komplit('+product_row+')" class="product-name" name="product[' + product_row + '][name]" /><input type="hidden" name="product[' + product_row + '][product_id]" /><input type="hidden" name="product[' + product_row + '][price]" /><input type="hidden" name="product[' + product_row + '][totalqty]" /></td>';
	html += '    <td class="left" id="option'+product_row+'"></td>';
	html += '    <td class="right"><select name="product[' + product_row + '][qty]" /></select></td>';
        html += '    <td class="right"><input type="text" name="product[' + product_row + '][harga_jual]" /></td>';
...
$("input[name='product["+coba+"][name]']").autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/productgudang/autocompletegudang&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request.term)+'&filter_gudang_id=' + $("select[name='gudang_asal']").val(),
			dataType: 'json',
    ...
    //alert(JSON.stringify(ui));
      var column=$(this).data("id");
        ...
        if(option_value['qty'] >= 1){
          ...
          html += '  <tr>';
          html +='<td class="center" style="width: 3px;"><img src="view/image/delete.png" onclick="$(\'#product-row'+coba+'\').remove()" style="cursor: pointer;" /></td>';

          html += '    <td class="left">'+ui.item['label']+'<input type="hidden" name="product[' + coba+ '][product_id]" value="'+ui.item['value']+'" /><input type="hidden" name="product[' + coba+ '][price]" value="'+ui.item['net_cost']+'" /><input type="hidden" name="product[' + coba+ '][totalqty]" value="'+ui.item['qty']+'" /></td>';
          ...
          html+='</select></td>';
          html += '    <td class="right"><input type="text" name="product[' + coba + '][harga_jual]" value="'+ui.item['price']+'" /></td>';
...
function simpan(){
  ...
  for(i=0;i<product_row;i++){
    ...
    qty=$("select[name='product["+i+"][qty]']").val();
