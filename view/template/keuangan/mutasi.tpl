      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            <h3 class="box-title">Mutasi Bank <?php echo $bank['name']; ?><br>
              ...
              <small>Saldo Akhir: <?php echo $totalsaldo; ?></small><br>
            </h3>
              ...
              <div class="col-md-3">
                <label>Type</label>
                            <select name="filter_jenis" class="select-ads">
                              ...
                              ?>
                              <!--<option value="2000">Pembelian</option>
                              <option value="2000">Uang Muka Pembelian</option>
                              <option value="4">Uang Muka Penjualan</option>
                              <option value="31">Pembayaran Piutang</option>-->
                            </select>
              </div>
              <div class="col-md-3">
                <label>Saldo</label>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="left">
                         <a onclick="detail('<?php echo $product['invoice'] ?>','<?php echo $product['type'] ?>')" data-toggle="modal" data-target="#jurnal"><?php echo empty($product['linkterkait'])?$product['invoice']:$product['invoice']; ?></a>
                        
                        </td>
                        <td class="left">
                        <?php if($product['invoice']>=3000) { ?>
                        <a onclick="detail('<?php echo $product['invoice'] ?>','<?php echo $product['type'] ?>')" data-toggle="modal" data-target="#jurnal"><?php echo empty($product['linkterkait'])?'':$product['linkterkait']; ?></a>
                        <?php }else{ ?>
                        <a href="<?php echo $product['urlref'];?>" target="_blank"><?php echo $product['linkterkait']; ?></a>
                        <?php } ?>
                        </td>
                        <td><?php echo $product['akunlawan']; ?></td>
                        ...
                        <?php foreach ($product['action'] as $action) { ?>
                          <a href="<?php echo $action['href']; ?>" <?php echo $action['text'] == 'Lihat Jurnal' | $action['text']=='Lihat Referensi'?'target="_blank"':''; ?> class="badge bg-yellow"><?php echo $action['text']; ?></a>
                        <?php } ?>
                        </td>
                      </tr>
                      <?php } ?>
                      <?php } else { ?>
                      <tr>
                        <td class="center" colspan="9" style="color:red">Data tidak ditemukan. Silahkan filter tanggal</td>
                      </tr>
                      <?php } ?>
                    </tbody>
                    <tfoot>
...
function detail(id,type){
    $.ajax({
      url: 'index.php?route=keuangan/bank/jurnal&token=<?php echo $token; ?>&id=' +  encodeURIComponent(id)+'&type='+type,
      //dataType: 'json',
