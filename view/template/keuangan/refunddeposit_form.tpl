...
function simpan(){
  ...
  if(!($("input[name='nominal']").val())){
    error=true;
    em +='Jumlah refund tidak boleh kosong <br>';
  }
  if(!$.isNumeric( Number($("input[name='nominal']").val())) ) {
    error=true;
    em +='Jumlah refund harus berupa angka <br>';
  }
  if(Number($("input[name='nominal']").val()) <= 0) {
    error=true;
    em +='Jumlah refund harus lebih dari 0 <br>';
  }

  if(!$.isNumeric( Number($("input[name='biaya_bank']").val())) ) {
    error=true;
    em +='Nilai Biaya Bank harus berupa angka <br>';
  }
  if(Number($("input[name='biaya_bank']").val()) < 0) {
    error=true;
    em +='Nilai biaya bank harus lebih dari samadengan 0 <br>';
  }


  if(Number($("input[name='nominal']").val()) > Number($("#deposit").val())){
    error=true;
    em +='Jumlah refund melebihi jumlah deposit tersedia <br>';
  }
