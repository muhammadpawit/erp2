                    ...
                    <tr>
                     ...
                     <tr>
                      ...
                      <tr>
                           ...
                           <td>
                             <input type="text" class="form-control" name="alasan_pengembalian" ivalue="" >
                           </td>
                       </tr>
                       <tr>
...
function updatetotal(){
 ...
 // alert(tabungmr);
  ...
  //if(!error){
    ...
    $("input[name='quantity']").val(tabungmr);
  //}
...
function addModule() {
  ...
  html +='<td class="center" style="width: 3px;"><img src="view/image/delete.png" onclick="hapus('+product_row+')" style="cursor: pointer;" /></td>';
	...
	html += '    <td id="productname-'+product_row +'" class="left"><select style="width:300px;" data-id="'+product_row+'" name="product[' + product_row + '][product_id]" class="product form-control"></select><input type="hidden" name="pemilik" value="1"></td>';
   ...
   html += '    <td id="productname-'+product_row +'" class="left"><select style="width:300px;" data-id="'+product_row+'" name="product[' + product_row + '][tttk_id]" class="tttk form-control"></td>';
	
  html += '    <td class="right"><input type="text" class="form-control" name="product[' + product_row + '][quantity]" onblur="updatetotal()" value="0" /><input type="hidden" class="form-control" name="product[' + product_row + '][quantitymax]" onblur="updatetotal()" value="0" /><input type="hidden" class="form-control" name="product[' + product_row + '][quantitytersedia]" value="0" /></td>';
  html += '  </tr>';
	html += '</tbody>';
  ...
  $(function(){
    $(".tttk").select2({
      ajax: {
      url:"index.php?route=sale/tttkmr/autocompleteproduct&token=<?php echo $token; ?>",
      dataType: 'json',
      data: function (params) {
        return {
          q: params.term,
          customer_id:$(".customer").val(), // search term
          product_id:$("input[name='product["+$(this).data('id')+"][product_id]']").val(),
          tutup:$("select[name='product["+$(this).data('id')+"][tutup]']").val()
        };
      },
      delay: 250,
    ...
    theme:"bootstrap"
        ...
        if(id != undefined & id != null){
          ...
          //alert($(this).data('id'));
        $.ajax({
          url: 'index.php?route=sale/tttkmr/detailproduct&token=<?php echo $token; ?>&product_id=' + $("input[name='product["+$(this).data('id')+"][product_id]']").val()+'&tttk_id='+id+'&tutup='+$("select[name='product["+$(this).data('id')+"][tutup]']").val(),
          dataType: 'json',
          success: function(json) {
          console.log(JSON.stringify(json));
            if(json.total !=undefined){
              if(Number(json.total) > 0){
                ...
                $("input[name='product["+coba+"][quantitymax]']").val(json.total);
              }else{
                 hapus(coba);
                alert("Tabung tidak tersedia");
              }
            }else{
              hapus(coba);
              alert("Tabung tidak tersedia");
    ...
    theme:"bootstrap"
        ...
        if(id != undefined & id != null){
          ...
          coba=$(this).data('id');
        $.ajax({
          url: 'index.php?route=catalog/tabungmr/detail&token=<?php echo $token; ?>&product_id=' + id+'&customer_id='+customer_id,
          dataType: 'json',
          success: function(json) {
          console.log(JSON.stringify(json));
            if(json.quantity !=undefined){
             // if(Number(json.quantity) > 0){
                $("select[name='product["+coba+"][product_id]']").remove();
                addhtml='<input type="hidden" name="product['+coba+'][product_id]" value="'+id+'"><input type="text" class="form-control" readonly name="product['+coba+'][name]" value="'+json.namaproduct+'">';
                $("#productname-"+coba).html(addhtml);
                $("input[name='product["+coba+"][quantitytersedia]']").val(json.quantity);
              //}
            }else{
              hapus(coba);
              alert("Tabung tidak ditemukan");
            }
...
function simpan(){
  ...
  if(!$("input[name='alasan_pengembalian']").val()){
    error=true;
    em +="Alasan Pengembalian harus diisi <br>";
  }
  /*if(product_row == 0){
    error=true;
  ...
  for(i=0;i<product_row;i++){
    ...
    quantitytersedia=$("input[name='product["+i+"][quantitytersedia]']").val();
    quantitymax=$("input[name='product["+i+"][quantitymax]']").val();
    ...
    if(pid != undefined){
     ...
     /* if(Number(quantity) > Number(quantitytersedia)){
        error=true;
        errqty=true;
      }*/
      if(Number(quantity) > Number(quantitymax)){
        error=true;
        errqty=true;
      }
  ...
  if(error){
    ...
    if(errqty){
      em+= "Terdapat tabung dengan quantity tidak valid. <br>"
    }
