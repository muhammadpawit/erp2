      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            <h3 class="box-title">Alokasi Pembayaran Lokal</h3>
...
function simpan(){
  ...
  //  pid=$("select[name='product["+i+"][product_id]']").val();
    ...
    qty=Number(Math.round($("input[name='orders["+i+"][total]']").val()));
    x=Number(Math.round($("input[name='orders["+i+"][totaltagihan]']").val()));
    console.log(x);
    if(qty < 0){
      error=true;
      errtotal=true;
...
html += '    <td class="right"><input class="form-control" onblur="updatetotal()" type="text" name="orders[' + product_row + '][total]" /></td>';
  ...
  $(document).ready(function() {
    ...
    $(".nosurat").select2({
      ajax: {
      url:"index.php?route=pembelian/invoicepembeliandagang/autocompletepembayaran&token=<?php echo $this->request->get['token']; ?>",
      dataType: 'json',
    ...
    theme:"bootstrap"
      ...
      $.ajax({
        url: 'index.php?route=pembelian/invoicepembeliandagang/detail&token=<?php echo $this->request->get['token']; ?>&id=' + id,
        dataType: 'json',
        success: function(json) {

            console.log(json);
            sisa=Number(json.total)+Number(json.biayakirim) - Number(json.totalbayar);
