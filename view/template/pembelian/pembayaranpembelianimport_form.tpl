      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            <h3 class="box-title">Alokasi Pembayaran Import</h3>
              ...
              <div class="col-md-12">
                <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
                  <table class="table">
                   ...
                   <tr>
                       <td>Kurs</td>
                       <td><input type="hidden"  id=kurs name="kurs" class="form-control" value="0" readonly><span id="nilaikurs"></span></td>
                   </tr>
                    <tr>
                    ...
                    <table class="table table-bordered" id="list-product">
                      <thead>
                        <tr>
                            <th>No. Faktur</th>
                            <th>Sisa Tagihan</th>
                            <th>Alokasi Pembayaran<br>(Dalam USD)</th>
                        </tr>
                      </thead>
                      <tbody>
...
html += '    <td class="right"><input class="form-control" onblur="updatetotal()" type="text" name="orders[' + product_row + '][total]" /></td>';
  ...
  $(document).ready(function() {
    ...
    theme:"bootstrap"
      ...
      $.ajax({
        url: 'index.php?route=pembelian/invoicepembelianimport/detail&token=<?php echo $this->request->get['token']; ?>&id=' + id,
        dataType: 'json',
...
theme:"bootstrap"
  ...
  $(".deposit").select2({
    ajax: {
    url:"index.php?route=pembelian/pembayarandepositimport/autocomplete&token=<?php echo $token; ?>",
    //url: "index.php?route=pamerantoko/toko/autocomplete&token=<?php echo $this->request->get['token']; ?>",
    dataType: 'json',
    data: function (params) {
      return {
  ...
  theme:"bootstrap"
      ...
      if(id != undefined & id != null){
      $.ajax({
        url: 'index.php?route=pembelian/pembayarandepositimport/detail&token=<?php echo $this->request->get['token']; ?>&id=' + id,
        dataType: 'json',
        success: function(json) {
          //alert(JSON.stringify(json));
            //$("#deposit").after("");
