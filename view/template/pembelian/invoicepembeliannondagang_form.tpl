      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            <h3 class="box-title">Buat Invoice Pembelian Lokal Non Produk Dagang</h3>
                ...
                <div class="col-sm-6 col-xs-12">
                  <table class="table table-responsive" >
                    ...
                    <tr id="biaya-kirim">
                      <td>Biaya Kirim</td>
                      <td>
                        <input type="text" class="form-control" onblur="updatetotal()" name="biayakirim" value="0">
...
$(document).ready(function() {
  ...
  theme:"bootstrap"
    ...
    $.ajax({
      url: 'index.php?route=pembelian/pembeliankreditnondagang/detail&token=<?php echo $this->request->get['token']; ?>&vendor_id=' + id+'&jenisbarang='+jenisproduk,
      dataType: 'json',
      success: function(json) {
        if(json){
          ...
          html += '    <td class="right"><input class="form-control" type="text" onblur="updatetotal()"  name="product[' + product_detail_row + '][price]"  value="'+json.products[i].harga+'" /><input class="form-control" type="hidden" name="product[' + product_detail_row + '][hargapo]"  value="'+json.products[i].harga+'" /></td>';
            ...
            html += '    <td class="right"><input class="form-control" type="text" onblur="updatetotal()"  name="product[' + product_detail_row + '][total]" value="'+((Number(json.products[i].harga)+Number(json.products[i].ppn))*json.products[i].quantity)+'" readonly /></td>';
...
//  alert('ok');
  ...
  biayakirim=Number($("input[name='biayakirim']").val());
  dp=Number($("#dp").val());
  pajak=0;
  grandtotal=0;
  totalpembulatan=0;
      ...
      if($.isNumeric( Number(quantity) ) & $.isNumeric( Number(harga) ) & $.isNumeric( Number(pajak) )){
        ...
        if(Number(pjk) > 0){
            ...
            */
            ...
            //sub_total += Math.ceil(Number(harga)*Number(quantity));
            sub_total += Number(harga)*Number(quantity);
          //  pajak+= Math.ceil(Number(pjk)*Number(quantity));


          }else{
          //  sub_total += Math.ceil(Number(quantity) * (Number(harga)));
          ...
          totalpo += (Number(hargapo)*Number(quantity)) + (Number(pjkpo)*Number(quantity));
...
function simpan(){
  ...
  for(i=0;i<product_detail_row;i++ ){
    if($("input[name='product["+i+"][product_id]']").val() != undefined){
      ...
      statuspenerimaan=$("input[name='product["+i+"][statuspenerimaan]']").val();
