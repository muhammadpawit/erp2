...
html += '    <td class="right"><input class="form-control" onblur="updatetotal()" type="text" name="orders[' + product_row + '][total]" /></td>';
  ...
  $(document).ready(function() {
    $(".nosurat").select2({
      ajax: {
      url:"index.php?route=pembelian/invoicepembeliankredit/autocompletepembayaran&token=<?php echo $this->request->get['token']; ?>",
      dataType: 'json',
    ...
    theme:"bootstrap"
      ...
      $.ajax({
        url: 'index.php?route=pembelian/invoicepembeliankredit/detail&token=<?php echo $this->request->get['token']; ?>&id=' + id,
        dataType: 'json',
