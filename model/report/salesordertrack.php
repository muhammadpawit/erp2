...
*/
class ModelReportSalesordertrack extends Model {
  ...
  public function listSo($product_id,$gudang_id,$data){
    ...
    $list=array();
    foreach($q->rows as $r){
      $sj=$this->db->all('penjualan',array('no_so'  => $r['id']));
      $detailsj=array();
      foreach($sj as $s){
        //invoice
        $inv=$this->db->firstdetail('invoice',array(),array(),array('jenisinvoice'=>3,'jenispenjualan'=> 1,'status'=>array('<>',4),'referensi'=>$s['id']));
        $detailsj[]=array(
          'id'  => $s['id'],
          'no_sj' => $s['no_sj'],
          'date_added'  => date('d/m/y',strtotime($s['date_added'])),
          'invoice' => $inv
        );
      }
  ...
  public function getProductTerjualByGudang($data=array()){
    $sql="SELECT i.customer_id,c.name as namacustomer,i.gudang_id,g.nama,COALESCE(SUM(ip.pajak*ip.quantity),0) as pajak,COALESCE(SUM(ip.total),0) as total,COALESCE(SUM(ip.quantity),0) as quantity,COALESCE(SUM(ip.net_cost*ip.quantity),0) as net_cost,COALESCE(SUM(ip.price*ip.quantity),0) as price ";
    //$sql .=" FROM invoice_product ip JOIN invoice i ON(ip.sales_order_id=i.id) JOIN product p ON(ip.product_id=p.product_id) JOIN gudang g ON(i.gudang_id=g.gudang_id) WHERE i.status <> 4 AND (jenispenjualan=1 OR i.jenispenjualan=2)";
    $sql .= " FROM sales_order_product ip JOIN sales_order i ON(ip.sales_order_id=i.id) lEFT JOIN customer c ON(i.customer_id=c.customer_id) JOIN product p ON(ip.product_id=p.product_id) JOIN gudang g ON(i.gudang_id=g.gudang_id) WHERE i.status <> 4 AND ip.status_pengiriman <> 4 ";
    ...
    $this->load->model('user/user');
      ...
      $sql .=" AND i.sales='".$this->user->getId()."'";
		}
