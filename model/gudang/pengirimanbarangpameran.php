<?php
class ModelGudangPengirimanbarangpameran extends Model {
  /*
  Status
  0. belum diterima
  1. sudah diterima
  ...
  */
  public function getTransferitem($order_id){
    ...
    $this->load->model('gudang/product');
    //product
    $sql2="SELECT pc.product_id,pc.product_option_id,p.name,pc.status,pc.quantity,pc.quantity_actual,pc.harga_jual from ".DB_PREFIX."pengiriman_barang_pameran_product pc LEFT JOIN ".DB_PREFIX."product p ON(pc.product_id=p.product_id) WHERE order_id='".$order_id."'";
  ...
  public function getTransferitems($data=array(),$permission=false){
      $sql="select t.order_id,t.invoice_no,t.date_added,t.status,t.total,t.totalterima,t.qtykirim,t.qtyterima,g.nama as asal,tj.lokasi as gudang_tujuan,t.keterangan from ".DB_PREFIX."pengiriman_barang_pameran t LEFT JOIN ".DB_PREFIX."gudang g ON(t.gudang_asal=g.gudang_id)  LEFT JOIN ".DB_PREFIX."pameran tj ON(t.tujuan=tj.pameran_id) ";
    ...
    $sql="select COUNT(*) as total from ".DB_PREFIX."pengiriman_barang_pameran t LEFT JOIN ".DB_PREFIX."gudang g ON(t.gudang_asal=g.gudang_id)  LEFT JOIN ".DB_PREFIX."pameran tj ON(t.tujuan=tj.pameran_id) ";
    if (!empty($data['filter_gudang_asal'])) {
      $sql .= " WHERE t.gudang_asal= '" . $data['filter_gudang_asal'] . "'";
    }
    else{
    ...
    $this->db->insert('pengiriman_barang_pameran',$trans);
    $order_id=$this->db->getLastId();

    $tgl=explode('-',$trans['date_added']);
    $invoice_no='SJ-PBP-'.$trans['user_id'].'-'.$tgl[0].'-'.$tgl[1].'-'.$order_id;
    ...
    $qty=0;
      ...
      $data['quantity'] = $data['qty'];
          ...
          'order_id'  => $order_id,
  			...
  			$this->db->insert('pengiriman_barang_pameran_product',$transprod);
      ...
      $this->load->model('pamerantoko/pameran');
			$tuj=$this->model_pamerantoko_pameran->getPameran($pro['tujuan']);
      $gud=$this->model_catalog_gudang->getGudang($pro['gudang_asal']);
			...
			if(!empty($data['product_option_id'])){
				...
				$kartustok=array(
					...
					'ket'	=> 'Pengiriman barang pameran ke '.$tuj['lokasi'].' oleh user '.$this->user->getName(),
					'saldo'	=> $update,
    ...
    $this->db->query("UPDATE ".DB_PREFIX."pengiriman_barang_pameran SET invoice_no='".$invoice_no."',total='".$total."',qtykirim='".$qty."' WHERE order_id='".$order_id."'");

    $act=array(
      'activity'	=> 'Input pengiriman barang  dari gudang '.$gud['nama'].' ke pameran '.$tuj['lokasi'].', nomor  '.$invoice_no,
  ...
  public function cancelTransfer($order_id){
    ...
    $this->db->query("UPDATE ".DB_PREFIX."pengiriman_barang_pameran SET status=3 WHERE order_id='".$order_id."'");
    $this->db->query("UPDATE ".DB_PREFIX."pengiriman_barang_pameran_product SET status=3 WHERE order_id='".$order_id."'");
      ...
      $gud=$this->model_catalog_gudang->getGudang($pro['gudang_asal']);
			...
			if(!empty($data['product_option_id'])){
				...
				$kartustok=array(
					...
					'stokmasuk'	=> $stokmasuk,
					'stokkeluar'	=> $stokkeluar,
					'ket'	=> 'Pembatalan pengiriman barang pameran ke '.$tuj['lokasi'].' oleh user '.$this->user->getName(),
					'saldo'	=> $update,
					'quantityawal'	=> $opt['quantity'],
    ...
    $act=array(
      'activity'	=> 'Pembatalan pengiriman barang pameran  dari gudang '.$gud['nama'].' ke pameran '.$tuj['lokasi'].', nomor  '.$pro['invoice_no'],
      'menu'	=> 'Transfer Item'
    );
    $this->user->addUserActivity($act);
  }
