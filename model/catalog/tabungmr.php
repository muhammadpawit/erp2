<?php
class ModelCatalogTabungmr extends Model {
	public function addTabung($data) {
		...
		$this->db->insert('tabung_mr',$tabung);
	...
	public function editTabung($id, $data) {
		...
		$this->db->update('tabung_mr',$tabung,$where);
	...
	public function getTabungs($data = array()) {
		$sql = "SELECT t.*,t.status,o.name as ukuran,c.name as namecustomer,p.name as namaproduct,c.title FROM " . DB_PREFIX . "tabung_mr t LEFT JOIN product_options o ON(t.ukuran_tabung=o.product_options_id) LEFT JOIN ".DB_PREFIX."customer c ON(t.customer_id=c.customer_id) LEFT JOIN ".DB_PREFIX."product p ON(t.product_id=p.product_id)  WHERE t.hapus=0 ";
	...
	public function getTabung($tabung) {
		$query = $this->db->query("SELECT t.*,o.name as namaukuran ,pd.name as namaproduct,c.name FROM " . DB_PREFIX . "tabung_mr t JOIN product_options o ON(t.ukuran_tabung=o.product_options_id) LEFT JOIN ".DB_PREFIX."customer c ON(t.customer_id=c.customer_id) LEFT JOIN product pd ON(t.product_id = pd.product_id) WHERE id='".$tabung."' ");

		return $query->row;
	}
	public function getTabungByProduct($product_id,$customer_id) {
		$query = $this->db->query("SELECT t.*,o.name as namaukuran ,pd.name as namaproduct,c.name FROM " . DB_PREFIX . "tabung_mr t LEFT JOIN product_options o ON(t.ukuran_tabung=o.product_options_id) LEFT JOIN ".DB_PREFIX."customer c ON(t.customer_id=c.customer_id) LEFT JOIN product pd ON(t.product_id = pd.product_id) WHERE t.product_id='".$product_id."' AND t.customer_id='".$customer_id."'");

		return $query->row;
	}

	public function getTotalTabungs($data=array()) {
		$sql = "SELECT COUNT(*) as total FROM " . DB_PREFIX . "tabung_mr t LEFT JOIN product_options o ON(t.ukuran_tabung=o.product_options_id)  WHERE t.hapus=0 ";
		if(isset($data['filter_pemilik'])){
			if(!empty($data['filter_pemilik'])){
				$sql .="  AND t.customer_id='".$data['filter_pemilik']."' ";
			}
	...
	public function getAsets($column=array(),$join=array(),$where=array(),$order=array(),$limit=0,$offset=null){
		return $this->db->alljoin('tabung_mr',$column,$join,$where,$order,$limit,$offset);
	}
}
?>
