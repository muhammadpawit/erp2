<?php
class ModelCatalogTitle extends Model {
	...
	public function deleteOptions($id) {
		$sql="UPDATE ".DB_PREFIX."title SET hapus=1 WHERE id='".$id."'";
		$this->db->query($sql);
	}

	public function getOption($id) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."title WHERE id = '" . (int)$id . "' AND hapus=0");

		return $query->row;
	}

	public function getOptions($data=array()) {
		$sql = "SELECT * FROM ".DB_PREFIX."title WHERE hapus=0";
	...
	public function getTitle($id){
		if($id > 0){
			$sql = "SELECT * FROM ".DB_PREFIX."title WHERE id='".$id."'";
			$query = $this->db->query($sql);
			if(!empty($query->row)){
