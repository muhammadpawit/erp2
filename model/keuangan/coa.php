...
class ModelKeuanganCoa extends Model {
	...
	public function getCategory($category_id) {
		//$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "coamnb WHERE kode_rek = '" . (int)$category_id . "'");
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "coamnb WHERE category_id = '" . (int)$category_id . "'");

		return $query->row;
	}
	
	public function getcoa($array) {
		$sql="SELECT DISTINCT * FROM " . DB_PREFIX . "coamnb WHERE kode_rek in ('1551','1552','1553','1554','2501','2502','2503','2504') order by name ASC";
	...
	public function getAllCategories($data=array()) {
			...
			if(isset($data['filter_type'])){
				if (!empty($data['filter_type'])) {
			    	...
			    	$sql .= " AND type='".$data['filter_type']."' ";
						...
						$sql .= " AND type IN(6,8)";
						$sql .=" OR kode_rek BETWEEN '5101' AND '5106' OR kode_rek='5099' ";
					}
			  }
			}
			...
			$sql .= " ORDER BY kode_rek, name ASC";
			if (isset($data['start']) || isset($data['limit'])) {
				if ($data['start'] < 0) {
					$data['start'] = 0;
				}
	...
	public function addSaldo($data){
		...
		$this->db->update('coamnb',$saldo,array('kode_rek'=>$data['kode_rek']));

		//cek
		$cek=$this->db->first('saldoawal',array('kode_rek'=>$data['kode_rek'],'tanggal'=>$data['tanggal']));
		if(!empty($cek)){
			$saldoawal=array(
				'tanggal'	=> $data['tanggal'],
			...
			$this->db->insert('saldoawal',$saldoawal);
		}else {
			$this->db->delete('saldoawal',array('kode_rek'=>$data['kode_rek'],'tanggal'=>$data['tanggal']));
			$saldoawal=array(
				'tanggal'	=> $data['tanggal'],
				'kode_rek'	=> $data['kode_rek'],
				'hapus'	=> 0,
