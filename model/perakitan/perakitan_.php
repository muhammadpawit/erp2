...
class ModelPerakitanPerakitan extends Model {
	...
	public function getDetail($id){
		$sql ="SELECT * FROM perakitan_detail WHERE id_perakitan='$id' ";
		$data=$this->db->query($sql);
		return $data->rows;
	}
	...
	public function simpanproses($data){
		...
		// HPP average lama
		/*
		$hppproduklama=$sa['net_cost']*$sa['quantity'];
		$hppprodukbaru=$sa['net_cost']+$ncc;
		$qtytotal = $sa['quantity'] + $data['qtyperakitan'];
		$hppavg = ($hppproduklama+$hppprodukbaru)/$qtytotal;
		$this->updatenetcostdariperakitan($data['gudang_id'],$data['product_id'],$hppavg);
		*/
		$hppakhir=0;
