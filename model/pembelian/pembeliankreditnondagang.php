<?php
class ModelPembelianPembeliankreditnondagang extends Model {
  public function addPembelian($data){
    ...
    $this->db->insert('pembelian_kreditnondagang',$p);
    $id=$this->db->getLastId();
    $data['id'] = $id;
    $data['status']=1;
    $no_surat='PO-'.$this->user->getId().'-'.date('Y',time()).'-'.date('m',time()).'-'.$id;
    $no_dokumen='LBA-'.$this->user->getId().'-'.date('Y',time()).'-'.date('m',time()).'-'.$id;
    $this->db->update('pembelian_kreditnondagang',array('no_po'  => $no_surat,'no_dokumen'=>$no_dokumen),array('id' => $id));
  ...
  public function addProduct($data){
    ...
    foreach($data['products'] as $d){
      ...
      $this->db->insert('pembelian_produk_kreditnondagang',$prod);
  ...
  public function barangdatang($data){
    ...
    */
    ...
    $this->db->update('suratjalan_pembeliannondagang',array('total'  => $total,'totalquantity' => $totalquantity),array('no_suratjalan'=>$data['no_suratjalan']));
    $this->addBiaya($data);
  ...
  public function updatePermintaan($data,$where){
    ...
    $this->db->update('pembelian_kreditnondagang',$data,$where);
  }
  public function getPermintaanPembelians($column=array(),$join=array(),$leftjoin=array(),$where=array(),$order=array(),$limit=0,$offset=null){
    return $this->db->alljoins('pembelian_kreditnondagang',$column,$join,$leftjoin,$where,$order,$limit,$offset);
  }

  public function totalPermintaans($where,$join=array(),$leftjoin=array()){
		return $this->db->countAll('pembelian_kreditnondagang',$where,$join,$leftjoin);
	}

  public function getBarangdatangs($column=array(),$join=array(),$leftjoin=array(),$where=array(),$order=array(),$limit=0,$offset=null){
    return $this->db->alljoins('suratjalan_pembeliannondagang',$column,$join,$leftjoin,$where,$order,$limit,$offset);
  }

  public function totalBarangdatangs($where,$join=array(),$leftjoin=array()){
		return $this->db->countAll('suratjalan_pembeliannondagang',$where,$join,$leftjoin);
	}

  public function getPermintaanPembelian($column=array(),$join=array(),$where=array()){
    return $this->db->firstdetail('pembelian_kreditnondagang',$column,$join,$where,array());
  }

  public function getPermintaanPembelianProduct($where){
    return $this->db->all('pembelian_produk_kreditnondagang',$where,array(),0,null);
  }
  ...
  public function getPermintaanPembelianProductDetail($where){
    return $this->db->firstdetail('pembelian_produk_kreditnondagang',array(),array(),$where,array());
  }
  ...
  public function getPoTanpaInvoice($vendor_id,$jenisbarang){
    ...
    $sql="SELECT pi.*,p.no_po,p.status as statuspenerimaan,p.sub_total,p.diskon as diskontotal  FROM pembelian_produk_kreditnondagang pi JOIN pembelian_kreditnondagang p ON(pi.pembelian_id=p.id) WHERE vendor_id='".$vendor_id."' AND p.status <> 3  AND p.jenis_barang='".$jenisbarang."'";
    $result=$this->db->query($sql);
    $hasil=array();
    foreach($result->rows as $r){
      //$ditagih=$this->model_pembelian_invoicepembeliankredit->getPermintaanPembelianProduct(array('p'=>$i['invoice_id']));
      $ditagih=$this->db->query("SELECT COALESCE(SUM(quantity),0) as total FROM invoice_pembeliannondagang_product ip JOIN invoice_pembeliannondagang i ON(ip.invoice_id=i.id) WHERE (i.status <> 5 AND i.status <> 3) AND ip.po_product_id='".$r['id']."'");
  ...
  //  $this->log->write(json_encode($result->rows));
    return $hasil;
  }
  ...
  public function tutuppo($id){
    ...
    if($pembelian['status'] == 2){
      $sql="SELECT pi.*,p.no_po FROM pembelian_produk_kreditnondagang pi JOIN pembelian_kreditnondagang p ON(pi.pembelian_id=p.id) WHERE pembelian_id='".$id."'";
      $result=$this->db->query($sql);
      $totalinv=0;
      foreach($result->rows as $r){
        ...
        //  $ditagih=$this->db->query("SELECT COALESCE(SUM(quantity),0) as total FROM invoice_pembelian_product ip JOIN invoice_pembelian i ON(ip.invoice_id=i.id) WHERE (i.status <> 5 AND i.status <> 3) AND ip.po_product_id='".$r['id']."'");
          ...
          if($total > 0){
            $this->db->update('pembelian_produk_kreditnondagang',array('quantity'=>$total),array('id'=>$r['id']));
          }else{
            $this->db->update('pembelian_produk_kreditnondagang',array('quantity'=>$r['quantityterima']),array('id'=>$r['id']));
          }
        }
      ...
      $update=$this->db->query("SELECT COALESCE(SUM(quantity*harga),0) as subtotal,COALESCE(SUM(ppn*quantity),0) as pajak FROM pembelian_produk_kreditnondagang WHERE pembelian_id='".$id."'");
      ...
      $this->db->update('pembelian_kreditnondagang',array('status'=>5,'sub_total'=>$subtotal,'pajak'=>$pajak,'total_pembelian'=>$totalbaru),array('id'=>$id));
      return true;
    }else{
      return false;
    }
