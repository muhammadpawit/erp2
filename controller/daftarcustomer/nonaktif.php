<?php
class ControllerDaftarcustomerNonaktif extends Controller {
	private $error = array();
	// baru 29 Juni 2020
	public function serversidebaru(){
		$this->load->model('daftarcustomer/nonaktif');
		$cust=$this->model_daftarcustomer_nonaktif->test();
	...
	private function getList() {
		...
		$this->data['customers'] = array();
		$daftarprov=$this->model_daftarcustomer_nonaktif->getsettingprov($this->user->getId());
		foreach($daftarprov as $dp){
			$pro[]=$dp['provinsi'];
		}
		$daftarcat=$this->model_daftarcustomer_nonaktif->getsettingcat($this->user->getId());
		foreach($daftarcat as $dp){
			$cats[]=$dp['category'];
		}
		$cek=$this->model_daftarcustomer_nonaktif->get($this->user->getId());
		if(empty($cek['setting'])){
			$data = array(
				'customer.name'	  => array('LIKE',$filter_name),
				'customer.alamat'	  => array('LIKE',$filter_alamat),
				'customer.sales'	=> ($filter_sales != null)?$filter_sales:array('>=',0),
			...
			$data = array(
				...
				'customer.hapus'	=> array('<',1),
				'customer.customer_id'	=> ($filter_customer_id != null)?$filter_customer_id:array('>=',0),
				'customer.country'=>array('IN',implode(",",$pro)),
				'customer.customer_group_id'=>array('IN',implode(",",$cats)),
				//'customer.customer_group_id'	=> ($filter_customer_group != null)?$filter_customer_group:array('>=',0)
			);
		}
		...
		$results = $this->model_daftarcustomer_nonaktif->getVendors($data,$orders,$limit,$offset);
		//echo "<pre>";print_r($results);exit;
		$product_total = $this->model_daftarcustomer_nonaktif->totalVendors($data);

		$this->load->model('sale/invoice');
		$this->load->model('user/user');
		$editlimit = $this->model_user_user->getAksesData($this->user->getId(),11);
		$no=1;
		$hari=$this->model_daftarcustomer_nonaktif->getlamahari($this->user->getId());
		//echo "<pre>";print_r($hari);exit();
		$this->data['lamahari']=$hari;
		foreach ($results as $result) {
			$action = array();
			...
			$this->data['customers'][] = array(
				'no'=>$no++,
				'invterakhir' => ($tglinvterakhir!=null)?date('Y-m-d',strtotime($tglinvterakhir)):0,
				'tglinvterakhir' => ($tglinvterakhir!=null)?date('d/m/Y',strtotime($tglinvterakhir)):'-',
				'customer_id' => $result['customer_id'],
				'customer_group'	=> $result['customer_group'],
				'name'        => $title.' '.$result['name'],
				'alamat'        => $result['alamat'],
		...
		/*
		if(empty($cek['setting'])){
			$this->template = 'daftarcustomer/nonaktif_listall.tpl';
		}else{
			$this->template = 'daftarcustomer/nonaktif_list.tpl';
		}*/
		$this->template = 'daftarcustomer/nonaktif_listall.tpl';
		$this->children = array(
			'common/header',
	...
	private function getForm() {
		...
		if (!empty($this->request->post)) {
			...
			$this->data['sapaan'] = $this->request->post['title'];
			...
			$this->data['countrys']=$this->request->post['country'];
			...
			$this->data['namapicdepantengah'] = $this->request->post['namapicdepantengah'];
			$this->data['namapicbelakang'] = $this->request->post['namapicbelakang'];
			$this->data['kodepos'] = $this->request->post['kodepos'];

		} elseif (!empty($option_info)) {
			//print_r($option_info);
			$this->data['title'] = empty($option_info['title'])?0:$option_info['title'];
			$this->data['sapaan'] = empty($option_info['title'])?0:$option_info['title'];
			...
			$this->data['namapicdepantengah']=empty($option_info['namapicdepantengah'])?'-':$option_info['namapicdepantengah'];
			$this->data['namapicbelakang']=empty($option_info['namapicbelakang'])?'-':$option_info['namapicbelakang'];
			$this->data['kodepos']=empty($option_info['kodepos'])?'-':$option_info['kodepos'];
		} else {
			$this->data['title'] = 0;
			$this->data['sapaan'] = 0;
			$this->data['area'] = 0;
			...
			$this->data['namapicdepantengah']='-';
			$this->data['namapicbelakang']='-';
			$this->data['kodepos']='-';
		}
		...
		$this->load->model('sale/customer_group');
		$this->data['customer_groups']=$this->model_sale_customer_group->getCustomerGroupsNew();
	...
	private function validateForm() {
		...
		if(isset($this->request->post['kodepos'])){
			if($this->request->post['kodepos']=="-"){
				$this->error['kodepos'] = 'Kode Pos harus diisi.';
			}
		}
