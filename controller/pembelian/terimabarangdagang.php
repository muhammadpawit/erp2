<?php
class ControllerPembelianTerimabarangdagang extends Controller {
	private $error=array();
	// baru 28 April 2020
	public function editsj(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      		...
      		$this->db->update('suratjalan_pembeliandagang',array('no_suratjalan'=>$this->request->post['no_suratjalan']),array('id'=>$this->request->post['id']));
      		$this->db->update('jurnal_umum',array('keterangan'=>'Peneriman Pembelian Produk Dagang Dengan Surat Jalan '.$this->request->post['no_suratjalan']),array('type'=>6,'ref'=>$this->request->post['id']));
			$this->session->data['success'] = 'Sukses: Data berhasil dirubah';
...
*/
    ...
    //'suratjalan_pembelian.hapus'	=> array('<',1),
		...
		$this->data['action']= $this->url->link('pembelian/terimabarangdagang/editsj', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$this->template = 'pembelian/barangdatang_editform.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				$action[] = array(
					...
					'href' => $this->url->link('pembelian/terimabarangdagang/editsj', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
		...
		$this->template = 'pembelian/terimabarangdagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
...
*/
	...
	public function tampil(){
		...
		$this->data['cancel']= $this->url->link('pembelian/terimabarangdagang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		//$this->data['action']= $this->url->link('pembelian/terimabarangdagang/terima', 'token=' . $this->session->data['token'] . $url.'&id='.$id, 'SSL');
		$this->data['cetak']= $this->url->link('pembelian/terimabarangdagang/cetak', 'token=' . $this->session->data['token'] . $url.'&id='.$id, 'SSL');
