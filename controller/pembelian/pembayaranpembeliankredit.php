<?php
class ControllerPembelianPembayaranpembeliankredit extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Pembayaran Pembelian Kredit');
		...
		$this->data['insert'] = $this->url->link('pembelian/pembayaranpembeliankredit/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
      ...
      'invoice_pembelian.status'	=> empty($filter_status)?array('<>',3):$filter_status,
		...
		$order=array(
			'invoice_pembelian.tglfaktur'	=> 'DESC',
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/pembayaranpembeliankredit/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			...
			$this->data['permintaans'][] = array(
				...
				'id'	=> $result['id'],
				'total'	=> $this->currency->format($result['total']+$result['biayakirim']),
		...
		$this->data['heading_title'] = 'Pembayaran Pembelian Lokal';
		...
		$pagination->url = $this->url->link('pembelian/pembayaranpembeliankredit', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/pembayaranpembeliankredit.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
		...
		$this->document->setTitle('Alokasi Pembayaran Pembelian Lokal');

		$this->load->model('pembelian/invoicepembeliankredit');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $no_po=$this->model_pembelian_invoicepembeliankredit->addPembayaran($this->request->post);

			$this->session->data['success'] = 'Sukses: Data Pembayaran pembelian lokal berhasil disimpan ';
			...
			$this->redirect($this->url->link('pembelian/pembayaranpembeliankredit', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		$url="";
		...
		$this->data['cancel']= $this->url->link('pembelian/pembayaranpembeliankredit', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/pembayaranpembeliankredit/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$this->template = 'pembelian/pembayaranpembeliankredit_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
		$this->document->setTitle('Pembayaran Pembelian Lokal');
		if (isset($this->request->get['filter_no_faktur'])) {
			$url .= '&filter_no_faktur=' . $this->request->get['filter_no_faktur'];
		}
		if (isset($this->request->get['filter_jenis_barang'])) {
		...
		$this->load->model('pembelian/invoicepembeliankredit');
		$column=array('invoice_pembelian.*','vendorlokal.name');
		$join=array();
		$join[]=array(
			'tablename'	=> 'vendorlokal',
			'secondtable'	=>'vendorlokal.id',
      ...
      'invoice_pembelian.hapus'	=> 0,
		...
		$this->template = 'pembelian/pembayaranpembeliankredit_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function batalkan(){
		...
		if (isset($this->request->get['id'])) {
			if(!empty($this->request->get['id'])){
      	$this->model_pembelian_invoicepembeliankredit->batalPembayaran($this->request->get['id']);

				$this->session->data['success'] = 'Sukses: Data Alokasi Pembayaran Pembelian Lokal berhasil dibatalkan.';
			}
		}
		$url='';
		if (isset($this->request->get['filter_no_faktur'])) {
