<?php
class ControllerPembelianPembelianimport extends Controller {
	private $error=array();
	...
	$leftjoin[]=array(
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Cetak PO',
				'href' => $this->url->link('pembelian/pembelianimport/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			/*$action[] = array(
				'text' => 'Email PO',
		...
		$pagination->url = $this->url->link('pembelian/pembelianimport', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/pembelianimport.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function cetak(){
      ...
      'permintaan_pembelian.hapus'	=> 0,
		...
		$this->template = 'pembelian/pebelianimport_cetak.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
