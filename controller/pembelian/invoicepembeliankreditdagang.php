...
class ControllerPembelianInvoicepembeliankredit extends Controller {
	...
	public function index() {
      ...
      'invoice_pembelian.status'	=> empty($filter_status)?array('>=',1):$filter_status,
		...
		if(!empty($filter_date_end)){
			$data['invoice_pembelian.tglfaktur']=array('>=',$filter_date_start,'<=',$filter_date_end);
		}else{
			$data['invoice_pembelian.tglfaktur']=array('>=',$filter_date_start);
		}
		...
		foreach ($results as $result) {
			...
			if($result['status'] == 5){
				$action[] = array(
					...
					'href' => $this->url->link('pembelian/invoicepembeliankredit/setujui', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Tolak Perubahan Harga',
					'href' => $this->url->link('pembelian/invoicepembeliankredit/tolak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
	...
	public function setujui(){
		...
		if (isset($this->request->get['id'])) {
			...
			$status=$this->model_pembelian_invoicepembeliankredit->getPermintaanPembelian(array(),array(),array('id'  => $this->request->get['id']));
			if($status['status'] == 5){
	      $this->model_pembelian_invoicepembeliankredit->updatePenjualan(array('status'=>1),array('id'	=> $this->request->get['id']));
	...
	public function batalkan(){
      ...
      $this->model_pembelian_invoicepembeliankredit->batalInvoice(array('id'	=> $this->request->get['id']));
			...
			$this->session->data['success'] = 'Sukses: Data Pembelian Lokal berhasil dibatalkan.';
			}
		}
		$url='';
		if (isset($this->request->get['filter_no_faktur'])) {
	...
	public function detail(){
		...
		if(isset($this->request->get['id'])){
			if(!empty($this->request->get['id'])){
				$id=$this->request->get['id'];
			$this->load->model('pembelian/invoicepembeliankredit');
			$column=array('invoice_pembelian.*');
			$join=array();
			...
			$data = array(
				'invoice_pembelian.id'	=> $id,
	      'invoice_pembelian.hapus'	=> 0,
				'invoice_pembelian.status'	=> array('<',3)
				//'start'                  => ($page - 1) * $this->config->get('config_admin_limit'),
				//'limit'                  => $this->config->get('config_admin_limit')
			);
