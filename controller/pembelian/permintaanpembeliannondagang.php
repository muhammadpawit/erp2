<?php
class ControllerPembelianPermintaanpembeliannondagang extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Permintaan Pembelian Non Produk Dagang');
		...
		$this->data['insert'] = $this->url->link('pembelian/permintaanpembeliannondagang/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
      ...
      'permintaan_pembelian.status'			=> $filter_status,
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/permintaanpembeliannondagang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			$action[] = array(
				'text' => 'Cetak',
				'href' => $this->url->link('pembelian/permintaanpembeliannondagang/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			...
			if($result['status'] == 1){
				...
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/permintaanpembeliannondagang/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Setujui',
					'href' => $this->url->link('pembelian/permintaanpembeliannondagang/setujui', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
		...
		$this->data['heading_title'] = 'Permintaan Pembelian Non Produk Dagang';
		...
		$pagination->url = $this->url->link('pembelian/permintaanpembeliannondagang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/permintaanpembeliannondagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
      ...
      $no_surat=$this->model_pembelian_permintaanpembelian->addPermintaanPembelian($this->request->post);
			...
			$this->session->data['success'] = 'Sukses: Data Permintaan Pembelian Non Produk Dagang Lokal berhasil disimpan dengan nomor surat '.$no_surat;
			...
			$this->redirect($this->url->link('pembelian/permintaanpembeliannondagang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('pembelian/permintaanpembeliannondagang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/permintaanpembeliannondagang/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$this->load->model('catalog/kelompokaset');
		...
		$this->template = 'pembelian/permintaanpembeliannondagang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
		$this->document->setTitle('Permintaan Pembelian Non Produk Dagang Lokal');
		$url = '';
		if (isset($this->request->get['filter_tanggal'])) {
			$url .= '&filter_tanggal=' . $this->request->get['filter_tanggal'];
		}
		...
		$data = array(
			...
			'permintaan_pembelian.jenis_barang'	=> array('<>',2),
	...
	//	print_r($prods);
		...
		$this->template = 'pembelian/permintaanpembeliannondagang_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
