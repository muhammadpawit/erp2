<?php
class ControllerPembelianBarangdatang extends Controller {
	private $error=array();
      ...
      'pembelian_kredit.status'	=> array('<>',3),
		...
		foreach ($results as $result) {
			...
			if($result['status'] != 1 ){
				$action[] = array(
					'text' => 'Terima',
					'href' => $this->url->link('pembelian/barangdatang/barangdatang', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/barangdatang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
		...
		$pagination->url = $this->url->link('pembelian/barangdatang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/barangdatangkredit.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
		...
		if(isset($this->request->get['id'])){
			if(!empty($this->request->get['id'])){
				...
				$this->redirect($this->url->link('pembelian/barangdatang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}
		}else{
			$this->redirect($this->url->link('pembelian/barangdatang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
...
*/
      ...
      'suratjalan_pembelian.hapus'	=> array('<',1),
		...
		$this->data['cancel']= $this->url->link('pembelian/barangdatang', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$this->template = 'pembelian/barangdatang_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function barangdatang(){
      ...
      $no_po=$this->model_pembelian_pembeliankredit->barangdatang($this->request->post);
		...
		$this->data['action']= $this->url->link('pembelian/barangdatang/barangdatang', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	//print_r($trans);
		...
		$this->template = 'pembelian/barangdatang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
