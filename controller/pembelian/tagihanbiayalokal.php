<?php
class ControllerPembelianTagihanbiayalokal extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Tagihan Biaya Lokal');
		...
		$this->data['insert'] = $this->url->link('pembelian/tagihanbiayalokal/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->data['insertpembayaran'] = $this->url->link('pembelian/tagihanbiayalokal/insertpembayarantagihan', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		*/
		...
		$column=array('tagihan_biaya_lokal.*');
		$join=array();
		/*$join[]=array(
			'tablename'	=> 'bank',
		...
		$product_total = $this->model_pembelian_biayapembeliankredit->totalTagihanBiayas($data);

		$results = $this->model_pembelian_biayapembeliankredit->getTagihanBiayas($column,$join,$data,$order,$limit,$offset);
	//	$this->load->model('keuangan/coa');
...
//	$this->load->model('pembelian/invoicepembelianimport');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/tagihanbiayalokal/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			...
			//riwayat pembayaran
		$pembayaran=$this->model_pembelian_biayapembeliankredit->getPembayaranTagihan(array('*'),array(),array('order_id'=>$result['id'],'status'=> 1),array('tgl_bayar'=>'ASC'));
			...
			$leftjoinbiaya=array();
			$biayas=$this->model_pembelian_biayapembeliankredit->getPermintaanPembelians(array(),$joinbiaya,$leftjoinbiaya,array('tagihan_id'=>$result['id']));
		...
		$pagination->url = $this->url->link('pembelian/tagihanbiayalokal', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/tagihanbiayalokal.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
      ...
      $no_po=$this->model_pembelian_biayapembeliankredit->addTagihanBiaya($this->request->post);

			$this->session->data['success'] = 'Sukses: Data Tagihan Biaya Pembelian Lokal  berhasil disimpan';
			...
			$this->redirect($this->url->link('pembelian/tagihanbiayalokal', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('pembelian/tagihanbiayalokal', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/tagihanbiayalokal/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'pembelian/tagihanbiayalokal_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function batalkan(){
      ...
      $this->model_pembelian_biayapembeliankredit->batalkanTagihan(array('id'	=> $this->request->get['id']));

			$this->session->data['success'] = 'Sukses: Data Tagihan Biaya Pembelian Lokal  berhasil dibatalkan.';
			}
		}
		$url = '';
		if (isset($this->request->get['filter_date_start'])) {
	...
	public function insertpembayarantagihan() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $no_po=$this->model_pembelian_biayapembeliankredit->addPembayaranTagihan($this->request->post);
		...
		$this->data['cancel']= $this->url->link('pembelian/tagihanbiayalokal', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/tagihanbiayalokal/insertpembayarantagihan', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'pembelian/pembayarantagihanbiayalokal_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function detailbiaya(){
		...
		if(isset($this->request->get['id'])){
			...
			*/
			...
			$hasil=$this->model_pembelian_biayapembeliankredit->getTagihanBiaya(array(),array(),$data);;
		}
	}
