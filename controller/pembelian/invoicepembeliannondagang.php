<?php
class ControllerPembelianInvoicepembeliannondagang extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Invoice Pembelian Lokal Non Produk Dagang');
		...
		$this->data['insert'] = $this->url->link('pembelian/invoicepembeliannondagang/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		*/
		...
		$data = array(
			...
			'invoice_pembeliannondagang.id'      =>$filter_no_faktur,
			'vendor_id'=> $filter_vendor,
			//'surat_id'=> $filter_no_surat,
			'jenisproduk'	=> $filter_jenis_barang,
      'invoice_pembeliannondagang.status'	=> empty($filter_status)?array('>=',1):$filter_status,
		...
		$order=array(
			...
			'invoice_pembeliannondagang.status'	=> 'ASC'
		);
		...
		foreach ($results as $result) {
			...
			if($result['status'] == 5){
				$action[] = array(
					...
					'href' => $this->url->link('pembelian/invoicepembeliannondagang/setujui', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Tolak Perubahan Harga',
					'href' => $this->url->link('pembelian/invoicepembeliannondagang/tolak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
		if($result['status'] == 1 & $result['statuspenerimaan'] == 0 /*& $result['inputpib'] != 1 & $result['statuspembayaranpib'] != 1 & $result['inputkursdatang'] != 1*/){
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/invoicepembeliannondagang/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/invoicepembeliannondagang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
		...
		$this->data['heading_title'] = 'Invoice Pembelian Lokal Non Produk Dagang';
		...
		$pagination->url = $this->url->link('pembelian/invoicepembeliannondagang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/invoicepembeliannondagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function setujui(){
		...
		if (isset($this->request->get['id'])) {
			...
			$status=$this->model_pembelian_invoicepembeliankreditnondagang->getPermintaanPembelian(array(),array(),array('id'  => $this->request->get['id']));
			if($status['status'] == 5){
	      $this->model_pembelian_invoicepembeliankreditnondagang->updatePenjualan(array('status'=>1),array('id'	=> $this->request->get['id']));
		...
		if (isset($this->request->get['page'])) {
			...
			$this->redirect($this->url->link('pembelian/invoicepembeliannondagang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
	}
	...
	public function tolak(){
		...
		if (isset($this->request->get['id'])) {
			...
			$status=$this->model_pembelian_invoicepembeliankreditnondagang->getPermintaanPembelian(array(),array(),array('id'  => $this->request->get['id']));
			if($status['status'] == 5){
	      $this->model_pembelian_invoicepembeliankreditnondagang->batalInvoice(array('id'	=> $this->request->get['id']));
	...
	public function insert() {
      ...
      $no_po=$this->model_pembelian_invoicepembeliankreditnondagang->addPenjualan($this->request->post);

			$this->session->data['success'] = 'Sukses: Data Invoice pembelian lokal non produk dagang berhasil disimpan ';
			$url='';
			if (isset($this->request->get['filter_no_faktur'])) {
				$url .= '&filter_no_faktur=' . $this->request->get['filter_no_faktur'];
			}
		...
		$this->data['cancel']= $this->url->link('pembelian/invoicepembeliannondagang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/invoicepembeliannondagang/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		*/
		...
		$this->template = 'pembelian/invoicepembeliannondagang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	//	$biayas=$this->model_pembelian_invoicepembeliankredit->getPermintaanPembelianBiaya(array('order_id'	=> $id));
		$pembayarans=$this->model_pembelian_invoicepembeliankreditnondagang->getPermintaanPembelianPembayaran(array('invoice_id'=>$id));
		/*$pbjoin[]=array(
			'tablename'=>'banks',
	...
	//	$this->data['biayas']=$biayas;
		...
		$this->template = 'pembelian/invoicepembeliannondagang_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function batalkan(){
      ...
      $this->model_pembelian_invoicepembeliankreditnondagang->batalInvoice(array('id'	=> $this->request->get['id']));
			...
			$this->session->data['success'] = 'Sukses: Data Pembelian Lokal Non Produk Dagang berhasil dibatalkan.';
			}
		}
		$url='';
		if (isset($this->request->get['filter_no_faktur'])) {
	...
	public function autocomplete(){
		...
		$this->load->model('pembelian/invoicepembeliankreditnondagang');
			...
			$results = $this->model_pembelian_invoicepembeliankreditnondagang->getPermintaanPembelians($column,$join,$join,$data,array(),$limit,$start);
	...
	public function detail(){
		...
		if(isset($this->request->get['id'])){
			if(!empty($this->request->get['id'])){
				$id=$this->request->get['id'];
			$this->load->model('pembelian/invoicepembeliankreditnondagang');
			$column=array('invoice_pembeliannondagang.*');
			$join=array();
	      ...
	      'invoice_pembeliannondagang.hapus'	=> 0,
			...
			$hasil=$this->model_pembelian_invoicepembeliankreditnondagang->getPermintaanPembelian($column,$join,$data);
		}
	}
