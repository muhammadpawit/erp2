<?php
class ControllerPembelianPermintaanpembeliandagang extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Permintaan Pembelian Produk Dagang');
		...
		$this->data['insert'] = $this->url->link('pembelian/permintaanpembeliandagang/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		*/
		...
		$column=array('permintaan_pembelian.*','divisi.name','gudang.nama','vendorlokal.name as nmvendor','permintaan_pembelian_product.product_name','permintaan_pembelian_product.quantity','permintaan_pembelian_product.keterangan as keteranganproduk');
		$join=array();
		$join[]=array(
			'tablename'	=> 'divisi',
		...
		$leftjoin[]=array(
			...
			'firsttable'	=>'permintaan_pembelian.vendor_id',
			'secondtable'	=> 'vendorlokal.id'
		);
		$leftjoin[]=array(
			'tablename'	=> 'permintaan_pembelian_product',
			'firsttable'	=>'permintaan_pembelian.id',
			'secondtable'	=> 'permintaan_pembelian_product.surat_id'
		);
		$data = array(
			'permintaan_pembelian.no_surat'      =>array('LIKE',$filter_no_surat),
			'permintaan_pembelian_product.keterangan'      =>array('LIKE',$filter_keterangan),
			'permintaan_pembelian.date(permintaan_pembelian.date_added)'      =>$filter_tanggal,
			'permintaan_pembelian.divisi_asal'=> $filter_divisi,
			'permintaan_pembelian.jenis_pembelian'	=> 2,
			'permintaan_pembelian.jenis_barang'	=> 2,
		...
		$product_total = $this->model_pembelian_permintaanpembelian->totalPermintaans($data,$join,$leftjoin);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/permintaanpembeliandagang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].'&gudang_id='.$result['gudang_id'].$url, 'SSL')
			);
			$action[] = array(
				'text' => 'Cetak',
				'href' => $this->url->link('pembelian/permintaanpembeliandagang/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			...
			if($result['status'] == 1){
				...
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/permintaanpembeliandagang/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Setujui',
					'href' => $this->url->link('pembelian/permintaanpembeliandagang/setujui', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].'&gudang_id='.$result['gudang_id'].$url, 'SSL')
				);
			}

			//referensi sales order
			$so=$this->model_pembelian_permintaanpembelian->getReferensi($result['id']);
			if(!empty($so)){
				$no_so='<a href="'.$this->url->link('sale/salesorder/tampil', 'token=' . $this->session->data['token'] . '&view=1&order_id=' . $so['sales_order_id'], 'SSL').'" target="_blank">'.$so['no_so'].'</a>';
				/* '<a href="'.$this->url->link('pembelian/permintaanpembelian/tampil', 'token=' . $this->session->data['token'] . '&id=' . $p['permintaan_id'].$url, 'SSL').'" target="_blank">'.$perm['no_surat'].'</a><br><br>'; */
			}else{
				$no_so='';
			}
			$this->data['permintaans'][] = array(
				...
				'keterangan'	=> $result['keteranganproduk'],
				'tgl_kebutuhan'	=> $result['tgl_kebutuhan'],
		...
		$this->data['heading_title'] = 'Permintaan Pembelian Produk Dagang';
		...
		$pagination->url = $this->url->link('pembelian/permintaanpembeliandagang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
		...
		$this->data['filter_divisi'] = $filter_divisi;

		$this->template = 'pembelian/permintaanpembeliandagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
      ...
      $no_surat=$this->model_pembelian_permintaanpembelian->addPermintaanPembelian($this->request->post);
			...
			$this->session->data['success'] = 'Sukses: Data Permintaan Pembelian Produk Dagang Lokal berhasil disimpan dengan nomor surat '.$no_surat;
			...
			$this->redirect($this->url->link('pembelian/permintaanpembeliandagang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('pembelian/permintaanpembeliandagang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/permintaanpembeliandagang/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$aktivas = $this->model_catalog_kelompokaset->getAktivas(array('hapus'	=> array('<',1)),array(),0,null);
		$this->data['aktivas']=$aktivas;
		*/
		$this->load->model('catalog/gudang');
		...
		$this->template = 'pembelian/permintaanpembeliandagang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
		$this->document->setTitle('Permintaan Pembelian Produk Dagang Lokal');
		$url = '';
		if (isset($this->request->get['filter_tanggal'])) {
			$url .= '&filter_tanggal=' . $this->request->get['filter_tanggal'];
		}
		...
		$column=array('permintaan_pembelian.*','divisi.name','permintaan_pembelian.date_added','permintaan_pembelian.status','permintaan_pembelian.gudang_id');
		$join=array();
		$join[]=array(
			'tablename'	=> 'divisi',
			'firsttable'	=>'permintaan_pembelian.divisi_asal',
	...
	//	print_r($prods);
		$so=$this->model_pembelian_permintaanpembelian->getReferensi($id);
		if(!empty($so)){
			$trans['no_so']='<a href="'.$this->url->link('sale/salesorder/tampil', 'token=' . $this->session->data['token'] . '&view=1&order_id=' . $so['sales_order_id'], 'SSL').'" target="_blank">'.$so['no_so'].'</a>';
			;
		}else{
			$trans['no_so']='';
		}
		...
		$this->template = 'pembelian/permintaanpembeliandagang_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function setujui(){
		$this->load->model('pembelian/permintaanpembelian');
		//echo $this->user->getId(); exit();
		$user=0;
		if(!empty($this->request->get['gudang_id'])){
			if($this->request->get['gudang_id']==1){
				$user=86;
			}else{
				$user=125;
			}
		...
		if (isset($this->request->get['id'])) {
			if(!empty($this->request->get['id'])){
			$this->model_pembelian_permintaanpembelian->updatePermintaan(array('status' => 2,'disetujui_oleh' =>$this->user->getId()),array('id'=> $this->request->get['id']));
