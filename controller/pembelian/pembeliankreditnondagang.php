<?php
class ControllerPembelianPembeliankreditnondagang extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Pembelian Kredit Non Produk Dagang');
		...
		$this->data['insert'] = $this->url->link('pembelian/pembeliankreditnondagang/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		*/
		...
		$column=array('pembelian_kreditnondagang.*','pembelian_produk_kreditnondagang.product_name','pembelian_produk_kreditnondagang.invoice_id as invoice','pembelian_produk_kreditnondagang.id as idproduk','pembelian_produk_kreditnondagang.quantity','pembelian_produk_kreditnondagang.quantityterima','permintaan_pembelian.no_surat','vendorlokal.name');
		...
		$join[]=array(
			...
			'firsttable'	=> 'pembelian_kreditnondagang.surat_id'
		);
		$join[]=array(
			'tablename'	=> 'vendorlokal',
			'secondtable'	=>'vendorlokal.id',
			'firsttable'	=> 'pembelian_kreditnondagang.vendor_id'
		);
		...
		$leftjoin[]=array(
			'tablename'	=> 'pembelian_produk_kreditnondagang',
			'secondtable'	=>'pembelian_produk_kreditnondagang.pembelian_id',
			'firsttable'	=> 'pembelian_kreditnondagang.id'
		);
		...
		$data = array(
			...
			'pembelian_kreditnondagang.vendor_id'=> $filter_vendor,
			'surat_id'=> $filter_no_surat,
			'pembelian_kreditnondagang.jenis_barang'	=> $filter_jenis_barang,
      'pembelian_kreditnondagang.status'	=> array('<>',3),
			//'start'                  => ($page - 1) * $this->config->get('config_admin_limit'),
			//'limit'                  => $this->config->get('config_admin_limit')
		);
		if($filter_status != '*'){
			if($filter_status == 0){
				$data['pembelian_kreditnondagang.status']=array('<',1);
			}else{
				$data['pembelian_kreditnondagang.status']=$filter_status;
			}
		}

		if(!empty($filter_date_end)){
			$data['pembelian_kreditnondagang.date_added']=array('>=',$filter_date_start,'<=',$filter_date_end);
		}else{
			$data['pembelian_kreditnondagang.date_added']=array('>=',$filter_date_start);
		}
		...
		$order=array(
			'pembelian_kreditnondagang.date_added'	=> 'DESC',
			'pembelian_kreditnondagang.id'	=> 'DESC',
			'pembelian_kreditnondagang.status'	=> 'ASC'
		);
		...
		$product_total = $this->model_pembelian_pembeliankreditnondagang->totalPermintaans($data,$join,$leftjoin);
		//countAll($tablename,$where=array(),$join=array(),$leftjoin=array())
		//$data['invoice_pembelian.status']=array('<>',4);
		$results = $this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelians($column,$join,$leftjoin,$data,$order,$limit,$offset);
		...
		foreach ($results as $result) {
			...
			if($result['status'] == 0 ){
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/pembeliankreditnondagang/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			if($result['status'] == 2 ){
				$action[] = array(
					'text' => 'Tutup PO',
					'href' => $this->url->link('pembelian/pembeliankreditnondagang/tutuppo', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
		...
		$action[] = array(
			'text' => 'Tampil',
			'href' => $this->url->link('pembelian/pembeliankreditnondagang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
		);
			$action[] = array(
				'text' => 'Cetak PO',
				'href' => $this->url->link('pembelian/pembeliankreditnondagang/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			$no_inv='';
		...
		$this->data['heading_title'] = 'Pembelian Kredit Non Produk Dagang';
		...
		$pagination->url = $this->url->link('pembelian/pembeliankreditnondagang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/pembeliankreditnondagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
      ...
      $no_po=$this->model_pembelian_pembeliankreditnondagang->addPembelian($this->request->post);
			...
			$this->redirect($this->url->link('pembelian/pembeliankreditnondagang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('pembelian/pembeliankreditnondagang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/pembeliankreditnondagang/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'pembelian/pembeliankreditnondagang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
		...
		$column=array('pembelian_kreditnondagang.*','permintaan_pembelian.no_surat','vendorlokal.name','permintaan_pembelian.jenis_barang');
		$join=array();
		$join[]=array(
			'tablename'	=> 'permintaan_pembelian',
			'secondtable'	=>'permintaan_pembelian.id',
		...
		$data = array(
			'pembelian_kreditnondagang.id'	=> $id,
      'permintaan_pembelian.hapus'	=> 0,
			//'start'                  => ($page - 1) * $this->config->get('config_admin_limit'),
			//'limit'                  => $this->config->get('config_admin_limit')
		);

		$trans=$this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelian($column,$join,$data);
		$prods=$this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelianProduct(array('pembelian_id'	=> $id));
	//	$biayas=$this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelianBiaya(array('order_id'	=> $id));
		...
		$this->template = 'pembelian/pembeliankreditnondagang_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function batalkan(){
		...
		if (isset($this->request->get['id'])) {
			if(!empty($this->request->get['id'])){
      $this->model_pembelian_pembeliankreditnondagang->updatePermintaan(array('status' => 3),array('id'	=> $this->request->get['id']));

			$this->session->data['success'] = 'Sukses: Data Pembelian Kredit Non Produk Dagang berhasil dibatalkan.';
			}
		}
			$url = '';
	...
	public function tutuppo(){
		...
		if (isset($this->request->get['id'])) {
			if(!empty($this->request->get['id'])){
			$pembelian=$this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelian(array(),array(),array('id'	=> $this->request->get['id']));

			if($pembelian['status'] == 2){
				//cek invoice
				$invpembelian=$this->model_pembelian_pembeliankreditnondagang->tutuppo($this->request->get['id']);
				if($invpembelian){
	...
	public function autocomplete(){
		...
		$this->load->model('pembelian/pembeliankreditnondagang');
			...
			$results = $this->model_pembelian_pembeliankreditnondagang->getPermintaanPembelians($column,$join,$join,$data,array(),$limit,$start);
	...
	public function detail(){
		...
		if(isset($this->request->get['vendor_id'])){
			...
			if(!empty($this->request->get['jenisbarang'])){
				$products=$this->model_pembelian_pembeliankreditnondagang->getPoTanpaInvoice($this->request->get['vendor_id'],$this->request->get['jenisbarang']);
			}
