<?php
class ControllerPembelianInvoicepembelianimport extends Controller {
	private $error=array();
	public function index() {
		...
		$this->data['insert'] = $this->url->link('pembelian/invoicepembelianimport/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
      ...
      'invoice_pembelian_import.status'	=> empty($filter_status)?array('>=',1):$filter_status,
		...
		if($result['status'] == 1 & $result['statuspenerimaan'] == 0 /*& $result['inputpib'] != 1 & $result['statuspembayaranpib'] != 1 & $result['inputkursdatang'] != 1*/){
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/invoicepembelianimport/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
		
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/invoicepembelianimport/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			...
			if($result['statuspenerimaan'] == 0 & $result['status'] != 3){
				$action[] = array(
					'text' => 'Estimasi Biaya',
					'href' => $this->url->link('pembelian/invoicepembelianimport/biaya', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}

			if($result['inputpib'] != 1 & $result['statuspembayaranpib'] != 1 & $result['status'] != 3){
				$action[] = array(
					'text' => 'PIB',
					'href' => $this->url->link('pembelian/invoicepembelianimport/pib', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}else{
				if($result['statuspembayaranpib'] != 1 & $result['status'] != 3){
					$action[] = array(
						'text' => 'Edit PIB',
						'href' => $this->url->link('pembelian/invoicepembelianimport/pib', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
					);
			...
			if($result['inputkursdatang'] != 1 & $result['status'] != 3){
				$action[] = array(
					'text' => 'Kurs Barang Datang',
					'href' => $this->url->link('pembelian/invoicepembelianimport/kurs', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}else{
				if($result['statuspenerimaan'] == 0 && $result['status'] != 3){
					$action[] = array(
						'text' => 'Edit Kurs Barang Datang',
						'href' => $this->url->link('pembelian/invoicepembelianimport/kurs', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
					);
				}
			}
		...
		$pagination->url = $this->url->link('pembelian/invoicepembelianimport', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/invoicepembelianimport.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
      ...
      $no_po=$this->model_pembelian_invoicepembelianimport->addPenjualan($this->request->post);
		...
		$this->data['action']= $this->url->link('pembelian/invoicepembelianimport/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'pembelian/invoicepembelianimport_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
