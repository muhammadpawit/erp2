<?php
class ControllerPembelianPembayaranpembelianbahanbaku extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Pembayaran Pembelian Bahan Baku');
		...
		$this->data['insert'] = $this->url->link('pembelian/pembayaranpembelianbahanbaku/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		*/
		...
		$data = array(
			...
			'invoice_pembelianbahanbaku.status'	=> empty($filter_status)?array('<>',3):$filter_status,
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('pembelian/pembayaranpembelianbahanbaku/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
		...
		$this->data['heading_title'] = 'Pembayaran Pembelian Bahan Baku';
		...
		$pagination->url = $this->url->link('pembelian/pembayaranpembelianbahanbaku', 'token=' . $this->session->data['token'] . $url . '&page={page}');
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'pembelian/pembayaranpembelianbahanbaku.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
		...
		$this->document->setTitle('Alokasi Pembayaran Pembelian Bahan Baku');

		$this->load->model('pembelian/invoicepembelianbahanbaku');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $no_po=$this->model_pembelian_invoicepembelianbahanbaku->addPembayaran($this->request->post);

			$this->session->data['success'] = 'Sukses: Data Pembayaran pembelian bahan baku berhasil disimpan ';
			...
			$this->redirect($this->url->link('pembelian/pembayaranpembelianbahanbaku', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		$url="";
		...
		$this->data['cancel']= $this->url->link('pembelian/pembayaranpembelianbahanbaku', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('pembelian/pembayaranpembelianbahanbaku/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$this->template = 'pembelian/pembayaranpembelianbahanbaku_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function tampil(){
      ...
      'invoice_pembelianbahanbaku.hapus'	=> 0,
		...
		$this->template = 'pembelian/pembayaranpembelianbahanbaku_info.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function batalkan(){
		...
		if (isset($this->request->get['id'])) {
			if(!empty($this->request->get['id'])){
      	$this->model_pembelian_invoicepembelianbahanbaku->batalPembayaran($this->request->get['id']);

				$this->session->data['success'] = 'Sukses: Data Alokasi Pembayaran Pembelian Bahan Baku berhasil dibatalkan.';
			}
		}
		$url='';
		if (isset($this->request->get['filter_no_faktur'])) {
