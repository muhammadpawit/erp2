<?php
class ControllerPembelianPembeliankreditdagang extends Controller {
	private $error=array();
	/*public function index() {
		$this->document->setTitle('Pembelian Lokal Produk Dagang');
		...
		$column=array('pembelian_kreditdagang.*','pembelian_produk_kreditdagang.product_name','pembelian_produk_kreditdagang.invoice_id as invoice','pembelian_produk_kreditdagang.id as idproduk','pembelian_produk_kreditdagang.quantity','pembelian_produk_kreditdagang.quantityterima','permintaan_pembelian.no_surat','vendorlokal.name','gudang.nama');
		...
		foreach ($results as $result) {
			...
			if($result['status'] == 0 ){
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('pembelian/pembeliankreditdagang/batalkan', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			if($result['status'] == 2 ){
				$action[] = array(
					'text' => 'Tutup PO',
					'href' => $this->url->link('pembelian/pembeliankreditdagang/tutuppo', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}
			if($result['status'] == 1 ){
				$action[] = array(
					'text' => 'Retur Pembelian',
					'href' => $this->url->link('pembelian/pembeliankreditdagang/retur', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
				);
			}

			$action[] = array(
				'text' => 'Edit Vendor',
				'href' => $this->url->link('pembelian/pembeliankreditdagang/editvendor', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);

		$action[] = array(
			'text' => 'Tampil',
			'href' => $this->url->link('pembelian/pembeliankreditdagang/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
		);
			$action[] = array(
				'text' => 'Cetak PO',
				'href' => $this->url->link('pembelian/pembeliankreditdagang/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
		...
		$pagination->url = $this->url->link('pembelian/pembeliankreditdagang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
		...
		$this->template = 'pembelian/pembeliankreditdagang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
