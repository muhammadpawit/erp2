<?php
class ControllerDaftarpolokalDaftarpolokal extends Controller {
	private $error=array();
	public function index() {
		$this->document->setTitle('Daftar Pembelian Lokal Produk Dagang');
		...
		*/
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Cetak PO',
				'href' => $this->url->link('daftarpolokal/daftarpolokal/cetak', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].$url, 'SSL')
			);
			...
			*/
			...
			$this->data['permintaans'][] = array(
				...
				'metode_pengiriman'	=> $result['metode_pengiriman'] == 1?'Antar Jemput ke Hanson':($result['metode_pengiriman'] == 2?'Tidak antar jemput':''),
		...
		$pagination->url = $this->url->link('daftarpolokal/daftarpolokal', 'token=' . $this->session->data['token'] . $url . '&page={page}');
		...
		$this->template = 'pembelian/daftarpolokal.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function cetak(){
		...
		$this->template = 'pembelian/pebeliankreditdagang_cetaktanpaharga.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
