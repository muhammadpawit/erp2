<?php
class ControllerEfakturPajakKeluaran extends Controller {
	private $error = array();
	...
	public function Export() {
		...
		if (isset($this->request->get['nomorfaktur'])) {
			$nofakt = $this->request->get['nomorfaktur'];
		} else {
			$nofakt = null;
		}
		...
		if (isset($this->request->get['nomorfaktur'])) {
			$url .= '&nomorfaktur=' . $this->request->get['nomorfaktur'];
		}
		if (isset($this->request->get['filter_gudang_id'])) {
		...
		$column=array('invoice.*','customer.name','customer.email','customer.telephone','customer.title','customer.date_added as register','customer.npwp','customer.alamat','customer.namanpwp','customer.alamatnpwp');
		$join=array();
		$join[]=array(
			'tablename'	=> 'customer',
		...
		$results = $this->model_sale_invoice->getPenjualans($column,$join,$data,$orders);
		...
		$this->data['sort_date_added'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.date_added' . $url, 'SSL');
		$this->data['sort_register'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=customer.date_added' . $url, 'SSL');
		$this->data['sort_tgl_lunas'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.tgllunas' . $url, 'SSL');
		$this->data['sort_customer'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=customer.name' . $url, 'SSL');
		$this->data['sort_tagihan'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.totaltagihan' . $url, 'SSL');
		$this->data['sort_bayar'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.totalbayar' . $url, 'SSL');
		$this->data['sort_invoice'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.id' . $url, 'SSL');
		$this->data['sort_metode_pembayaran'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.metode_pembayaran' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . '&sort=invoice.status' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('efaktur/pajakkeluaran', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->data['export'] = $this->url->link('efaktur/pajakkeluaran/export', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->data['sort'] = $sort;
		$this->data['order'] = $order;
		$this->data['nomorfaktur'] = $nofakt;

		if(!isset($this->request->get['print'])){
			$this->template = 'efaktur/pajakkeluaran_export.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
		}else{
			$this->template = 'efaktur/pajakkeluaran_cetak.tpl';
		}
	...
	private function getList() {
		...
		if(!isset($this->request->get['print'])){
			$this->template = 'efaktur/pajakkeluaran_list.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
