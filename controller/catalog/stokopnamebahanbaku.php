<?php
class ControllerCatalogStokopnamebahanbaku extends Controller {
	public function index() {
		$this->document->setTitle('Stok Opnae Bahan Baku');
    ...
    if (isset($this->request->get['filter_id'])) {
		...
		$this->data['insert'] = $this->url->link('catalog/stokopnamebahanbaku/insert', 'token=' . $this->session->data['token'], 'SSL');
      ...
      'start'                  => ($page - 1) * $this->config->get('config_admin_limit'),
		...
		$pagination->url = $this->url->link('catalog/stokopnamebahanbaku', 'token=' . $this->session->data['token'] . $url . '&page={page}');
		...
		$this->template = 'catalog/stokopnamebahanbaku.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/stokopnamebahanbaku', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('catalog/stokopnamebahanbaku', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('catalog/stokopnamebahanbaku/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'catalog/stokopnamebahanbaku_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm() {
    	/*if (!$this->user->hasPermission('modify', 'catalog/stokopnamebahanbaku')) {
