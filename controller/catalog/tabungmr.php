<?php
class ControllerCatalogTabungmr extends Controller {
	private $error = array();
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_tabungmr->addTabung($this->request->post);
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_tabungmr->editTabung($this->request->get['id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $gudang_id) {
				$this->model_catalog_tabungmr->deleteTabung($gudang_id);
			}
	...
	$this->data['insert'] = $this->url->link('catalog/tabungmr/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
	$this->data['delete'] = $this->url->link('catalog/tabungmr/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$product_total = $this->model_catalog_tabungmr->getTotalTabungs($data);

	$results = $this->model_catalog_tabungmr->getTabungs($data);
...
/*		$action[] = array(
		...
		$action[] = array(
			'text' => 'Kartu Stok',
			'href' => $this->url->link('catalog/tabungmr/kartustok', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, 'SSL')
		);
		/*$action[] = array(
				'text' => 'Koreksi Stok',
	...
	$pagination->url = $this->url->link('catalog/tabungmr', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	$this->template = 'catalog/tabungmr_list.tpl';
	$this->children = array(
		'common/header',
		'common/footer'
	);
	...
	private function validateDelete() {
		if (!$this->user->hasPermission('modify', 'catalog/tabungmr')) {
	...
	public function autocompleteprodso() {
		...
		$this->load->model('sale/tttkmr');
			...
			$data = array(
				...
				'tabung_mr.customer_id'	=> $filter_customer_id,
				'tttk_tabungmr.tttk_id'	=> $tttk,
				//'start'               => 0,
				//'limit'               => $limit
			);
			$offset=0;
			...
			$results = $this->model_sale_tttkmr->getPenjualanProducts($data);
	...
	public function kartustok() {
		...
		$this->document->setTitle('Kartu Stok Tabung MR');
		...
		$this->data['url'] = $this->url->link('catalog/tabungmr/kartustok', 'token=' . $this->session->data['token'] . $url, 'SSL');
   	...
   	if (isset($this->session->data['success'])) {
		...
		$order_total = $this->model_catalog_kartustoktabungmr->getTotalKartustoks($data);

		$results = $this->model_catalog_kartustoktabungmr->getKartustoks($data);
		$this->data['cancel'] = $this->url->link('catalog/tabungmr', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->data['heading_title'] = 'Kartu Stok Tabung MR';
		$this->data['token'] = $this->session->data['token'];
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('catalog/tabungmr/kartustok', 'token=' . $this->session->data['token'] . $url . '&pagekartu={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['tanggal']=$tanggal;
		$this->data['type']=$type;
		$this->template = 'catalog/kartustok_tabungmr.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function detail(){
		...
		if(isset($this->request->get['product_id'])){
			if(!empty($this->request->get['product_id'])){
				...
				$hasil=$this->model_catalog_tabungmr->getProduct($product_id,$customer_id);
