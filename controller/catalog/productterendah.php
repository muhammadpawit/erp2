<?php
class ControllerCatalogProductTerendah extends Controller {
	private $error = array();

	// baru 25 Agustus 2020
	public function setbelum(){
	...
	public function edittable(){
		...
		if($this->request->get['column']!="poin"){
			echo $this->currency->format($this->request->post['harga_terendah']);
		}else{
			echo $this->request->post['harga_terendah'];
		}
	...
	public function set(){
		...
		if (isset($this->request->get['periode'])) {
			$periode = $this->request->get['periode'];
		} else {
			$periode =null;
		}
       ...
       /* if (isset($this->request->get['filter_product_id'])) {
		...
		$results = $this->model_gudang_product->getdaftarbarang($filter_gudang_id);
		...
		//if(isset($result['product_gudang_id'])){
			...
			*/
			$hargarendah=$this->model_gudang_product->gethargarendah($result['product_id'],$filter_gudang_id,$date);
      		$this->data['products'][] = array(
				'gudang'   => $result['gudang_id'],
		...
		$pagination->url = $this->url->link('catalog/productterendah', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->data['date']=date('Y-m-d',strtotime($date . ' +1 days'));
		...
		if (isset($this->request->get['print'])) {
			$this->template = 'catalog/daftarhargaterendah_cetak.tpl';
		}else{
			$this->template = 'catalog/daftarhargaterendah_set.tpl';
		}
		$this->children = array(
			'common/header',
			'common/footer'
	...
	public function cetakstokpertanggal(){
		echo "<pre>";print_r($this->cetakexcelpertanggal());
		//$this->xlscreation_directtanggal();
	}
	...
	public function index() {
       ...
       /* if (isset($this->request->get['filter_product_id'])) {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
      		...
      		$this->data['products'][] = array(
				...
				'hargatanpaformat'   => $result['harga_terendah'],
		...
		if (isset($this->request->get['print'])) {
			...
			$this->template = 'catalog/daftarhargaterendah.tpl';
		}
		$this->children = array(
			'common/header',
			'common/footer'
