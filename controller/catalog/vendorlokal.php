<?php
class ControllerCatalogVendorlokal extends Controller {
	private $error = array();
	...
	public function inserthistory() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			//hanya untuk input riwayat saja
			echo "<pre>";print_r($this->request->post);exit;
			$this->model_catalog_vendorlokal->addhistorydepositVendor($this->request->post);

			$this->session->data['success'] = 'Data History Deposit Vendor Lokal berhasil ditambahkan.';
			$url = '';
			...
			$this->redirect($this->url->link('catalog/vendorlokal/deposit', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
		$url='';
		if (isset($this->request->get['filter_name'])) {
			$url .= '&filter_name=' . urlencode(html_entity_decode($this->request->get['filter_name'], ENT_QUOTES, 'UTF-8'));
		...
		$this->data['action'] = $this->url->link('catalog/vendorlokal/inserthistory', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->data['cancel'] = $this->url->link('catalog/vendorlokal/deposit', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->template = 'catalog/vendorlokalhistorydeposit_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getList() {
		...
		if (isset($this->request->get['page'])) {
   		...
   		$this->data['insert'] = $this->url->link('catalog/vendorlokal/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/vendorlokal/delete', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['excel'] = $this->url->link('catalog/vendorlokal', 'token=' . $this->session->data['token'] .'&excel=true'. $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Edit',
				'href' => $this->url->link('catalog/vendorlokal/update', 'token=' . $this->session->data['token'] . '&id=' . $result['id'], 'SSL')
			);
			$action[] = array(
				'text' => 'Deposit',
				'href' => $this->url->link('catalog/vendorlokal/deposit', 'token=' . $this->session->data['token'] . '&id=' . $result['id'], 'SSL')
			);

			$action[] = array(
				'text' => 'Contact',
				'href' => $this->url->link('catalog/vendorlokal/contact', 'token=' . $this->session->data['token'] . '&id=' . $result['id'], 'SSL')
			);
		...
		$pagination->url = $this->url->link('catalog/vendorlokal', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		if(isset($this->request->get['excel'])){
			...
			$this->template = 'catalog/vendorlokal_list.tpl';
		}
	...
	public function deposit() {
		...
		$this->data['inserthistory'] = $this->url->link('catalog/vendorlokal/inserthistory', 'token=' . $this->session->data['token'] . $url, 'SSL');
