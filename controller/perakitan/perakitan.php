...
class ControllerPerakitanPerakitan extends Controller {
	...
	public function cek(){
		...
		if(isset($this->request->get['product_id'])){
			if(!empty($this->request->get['product_id'])){
				...
				$detail=$this->model_catalog_product->getProduct($product_id);
			}
		}
		$hasil=array(
			'detail'	=> $detail,
	...
	public function batalkanperakitan(){
		...
		$this->load->model('perakitan/perakitan');
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				$si = $this->model_perakitan_perakitan->batalkanperakitan($this->request->post);
	...
	public function modaldetailbatalkan(){
		...
		$this->load->model('pembelian/pembelianimport');
			...
			$notif=null;
			$pro=0;
			$proses = $this->url->link('perakitan/perakitan/batalkanperakitan', 'token=' . $this->session->data['token'].$url, 'SSL');
			echo '
            ...
            <table class="table table-bordered" id="tbl">
               ...
               <tbody>';
				...
				// if($pro==2){
					...
					$act='<p class="text-center"><button type="submit" class="btn btn-success">Ya Batalkan Proses Perakitan</button>&nbsp;&nbsp;&nbsp;';
				// }else{
	...
	public function proses(){
		...
		$this->load->model('perakitan/perakitan');
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				$cek=$this->model_gudang_product->getProduct($post['product_id'],$post['gudang_id']);
				if(!empty($cek)){
					$si = $this->model_perakitan_perakitan->simpanproses($this->request->post);
				}else{
					$si = $this->model_perakitan_perakitan->simpanprosesnew($this->request->post);
				}
	...
	public function index() {
		...
		$this->data['batalkanproses'] = $this->model_user_user->getAksesData($this->user->getId(),12);
