...
class ControllerSaleCustomer extends Controller {
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Buku Piutang',
				'href' => $this->url->link('sale/customer/bukupiutang', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'].$url, 'SSL')
			);
 ...
 */
	...
	public function bukupiutang() {
		...
		$this->document->setTitle("Buku Piutang Customer");
		...
		$address_total = $this->model_sale_customer->getTotalBukuPiutangs($this->request->get['customer_id']);

		$results = $this->model_sale_customer->getBukuPiutangs($this->request->get['customer_id'],$data);
		//print_r($results);
		...
		foreach ($results as $result) {
			...
			$this->data['addresses'][] = array(
				'date_trans'    => date('d/m/y',strtotime($result['date_added'])),
				'saldomasuk'           => $this->currency->format($result['saldomasuk']),
				'saldokeluar'           => $this->currency->format($result['saldokeluar']),
				'ref'             => $result['referensi'],
				'keterangan'             => $result['urlref']=='sale/invoice'?'Tagihan No '.$result['referensi']:'',
				//'selected'       => isset($this->request->post['selected']) && in_array($result['id'], $this->request->post['selected']),
				'no_dokumen'             => $result['no_dokumen'],
				'urlref'	=> $this->url->link($result['urlref'].'/tampil', 'token=' . $this->session->data['token'] . '&id=' . $result['idref'].'&view=1&order_id='.$result['idref'], 'SSL'),
				'actions'	=> $action
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/customer/bukupiutang', 'token=' . $this->session->data['token'] . $url . '&pagealamat={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();


		$this->template = 'sale/bukupiutang_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
