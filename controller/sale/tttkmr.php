<?php
class ControllerSaleTttkmr extends Controller {
	private $error = array();
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_tttkmr->updateOrderPenjualan($this->request->post, array('id'=>$this->request->get['id']));
			...
			$this->redirect($this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	...
	private function getList() {
		...
		$this->data['url'] = $this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'] . $url, 'SSL');
		//$this->data['urllokasi'] = $this->url->link('pamerantoko/toko/autocomplete', 'token=' . $this->session->data['token'] , 'SSL');

   		$this->data['insert'] = $this->url->link('sale/tttkmr/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->data['delete'] = $this->url->link('sale/tttkmr/delete', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$column=array('tttk_mr.*','customer.title','customer.name','customer.alamat','customer.telephone','customer.email');
		$join=array();
		$join[]=array(
			'tablename'	=> 'customer',
			'firsttable'	=> 'tttk_mr.customer_id',
			'secondtable'	=> 'customer.customer_id',
		);
		if(!empty($filter_tanggal)){
			$order=array(
				'tttk_mr.date_added'	=> 'DESC',
			);
		}else{
			$order=array(
				'id'	=> 'DESC',
		...
		$data = array(
			'tttk_mr.id'	=> empty($filter_order_id)?array('>',0):$filter_order_id,
			'tttk_mr.customer_id'	=> empty($filter_customer_id)?array('>',0):$filter_customer_id,
			//'tttk_mr.date_added'	=> empty($filter_tanggal)?array('>','1901-01-01'):$filter_tanggal,
			'tttk_mr.date_added'	=> empty($filter_tanggal)?array('>','1901-01-01'):array('>=',$filter_tanggal,'<=',$filter_tanggalakhir),
			'tttk_mr.pengiriman'	=> empty($filter_shipping_method)?array('>',0):$filter_shipping_method,
			'tttk_mr.status'	=> empty($filter_status)?array('>=',1):$filter_status,

		);
		$offset=($page - 1) * $this->config->get('config_admin_limit');
		$limit=$this->config->get('config_admin_limit');

		$results = $this->model_sale_tttkmr->getPenjualans($column,$join,$data,$order,$limit,$offset);
		$product_total = $this->model_sale_tttkmr->totalPenjualans($data);
		//print_r($results);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Tampil',
				'href' => $this->url->link('sale/tttkmr/tampil', 'token=' . $this->session->data['token'] . '&view=1&order_id=' . $result['id'], 'SSL')
			);
			if($result['status'] == 1){
				$action[] = array(
					'text' => 'Batalkan',
					'href' => $this->url->link('sale/tttkmr/batal', 'token=' . $this->session->data['token'] . '&view=3&order_id=' . $result['id'], 'SSL')
				);
			}
			/*$action[] = array(
				'text' => 'Invoice',
		...
		$pagination->url = $this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'sale/tttkmr_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function autocomplete(){
		...
		$this->load->model('sale/tttkmr');
			...
			$results = $this->model_sale_tttkmr->getPenjualans(array(),array(),$data,array(),10,0);
	...
	public function tampil(){
		...
		$trans=$this->model_sale_tttkmr->getPenjualanDetail($column,$join,$data,array());
		...
		$trans['kernets']=$this->model_sale_tttkmr->getPenjualanKernets(array('tttk_id'	=> $order_id));
		//($column=array(),$join=array(),$where=array(),$order,$limit,$offset)
		$salesorder=$this->model_sale_salesorder->getPenjualans(array(),array(),array('tttk'=>$order_id,'status'	=> array('<>',4)),array(),0,null);
		...
		//print_r($salesorder);
		$products=$this->model_sale_tttkmr->getPenjualanProducts(array('tttk_id'	=> $order_id,'tabung_mr.customer_id'=>$trans['customer_id']));
		...
		$this->data['cancel']= $this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['suratjalan']= $this->url->link('sale/tttkmr/tampil', 'token=' . $this->session->data['token'] .'&order_id='.$order_id.'&view=2'. $url, 'SSL');
		$this->data['invoice']= $this->url->link('sale/tttkmr/tampil', 'token=' . $this->session->data['token'] .'&order_id='.$order_id.'&view=3'. $url, 'SSL');
		if($this->request->get['view'] == 1){
			$this->template = 'sale/tttkmr_info.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
		...
		if($this->request->get['view'] == 3){
			$this->template = 'sale/tttkmr_cetak.tpl';
		}
	...
	public function detail(){
		...
		if(isset($this->request->get['id'])){
			if(!empty($this->request->get['id'])){
				...
				$products=$this->model_sale_tttkmr->getPenjualanProducts(array('tttk_id'	=> $order_id));
	...
	public function batal() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ($this->request->get['order_id']) {
				...
				//print_r($this->request->post);
					$p=$this->model_sale_tttkmr->getPenjualan(array('id'=>$this->request->get['order_id']));
					if($p['status'] == 1){
			      $this->model_sale_tttkmr->cancelPenjualan($this->request->get['order_id'],$_POST['alasan_batal']);
		...
		$this->data['cancel']= $this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('sale/tttkmr/batal', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			//print_r($this->request->post);
      $order= $this->model_sale_tttkmr->addPenjualan($this->request->post);
		...
		*/
		...
		$this->data['cancel']= $this->url->link('sale/tttkmr', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('sale/tttkmr/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'sale/tttkmr_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
