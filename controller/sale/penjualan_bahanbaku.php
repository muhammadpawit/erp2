...
class ControllerSalePenjualanbahanbaku extends Controller {
	...
	private function getList() {
   	...
   	$this->data['insert'] = $this->url->link('sale/penjualanbahanbaku/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$column=array('penjualan.*','sales_order.no_so as nom_so','gudang.nama','customer.name','customer.alamat','customer.telephone','customer.email');
		$join=array();
		$join[]=array(
			'tablename'	=> 'customer',
		...
		$data = array(
			...
			'penjualan.date_added'	=> empty($filter_tanggal)?array('>','1901-01-01'):$filter_tanggal,
			'penjualan.total'	=> empty($filter_total)?array('>=',0):$filter_total,
			'penjualan.status'	=> empty($filter_status)?array('>',0):$filter_status,
			'penjualan.no_invoice'	=> $filter_shipping_method != null ?array('>',0):$filter_shipping_method,
		...
		foreach ($results as $result) {
			...
			/*if($result['status'] == 2){
				$action[] = array(
					'text' => 'Email Invoice',
					'href' => $this->url->link('sale/penjualanbahanbaku/email', 'token=' . $this->session->data['token'] . '&view=3&order_id=' . $result['order_gudang_id'], 'SSL')
				);
			}

			/*$action[] = array(
				'text' => 'Invoice',
				'href' => $this->url->link('sale/penjualanbahanbaku/tampil', 'token=' . $this->session->data['token'] . '&view=3&order_id=' . $result['order_gudang_id'], 'SSL')
			);*/
			/*$action[] = array(
				'text' => 'Surat Jalan',
				'href' => $this->url->link('sale/penjualanbahanbaku/tampil', 'token=' . $this->session->data['token'] . '&view=2&order_id=' . $result['id'], 'SSL')
			);*/
			...
			/*if($result['payment_status'] == 3 & $result['status_pengiriman'] == 2){
				$action[] = array(
					'text' => 'Selesai',
					'href' => $this->url->link('sale/penjualanbahanbaku/selesai', 'token=' . $this->session->data['token'] . '&order_gudang_id=' . $result['order_gudang_id'], 'SSL')
				);
	...
	public function tampil(){
		...
		$column=array('penjualan.*','customer.name','customer.telephone','customer.email','gudang.nama');
		$join=array();
		$join[]=array(
			'tablename'	=> 'customer',
			'firsttable'	=> 'penjualan.customer_id',
