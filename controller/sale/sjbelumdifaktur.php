<?php
class ControllerSalesjbelumdifaktur extends Controller {
	private $error = array();
	...
	function xlscreation_direct() {
		...
		$fileName = "SJ_Belum_Difakturkan_".$presentDate.".xlsx";
	...
	private function getList() {
		...
		$data += array(
			...
			//'penjualan_product.invoice_id'	=> empty($filter_invoice)?array('>=',0):$filter_invoice,
			'penjualan_product.no_so'	=> empty($filter_sales_order)?array('>=',0):$filter_sales_order,
			'penjualan_product.quantityreturn'=>array('=',0),
			//'penjualan.no_invoice'	=> $filter_shipping_method != null ?array('<>',):$filter_shipping_method,
			//'start'           => ($page - 1) * $this->config->get('config_admin_limit'),
			//'limit'           => $this->config->get('config_admin_limit')
		);
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/sjbelumdifaktur', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
