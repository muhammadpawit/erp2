<?php
class ControllerSalesogantung extends Controller {
	private $error = array();
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$action = array();
					...
					$action[] = array(
						'text' => 'cetak',
						'href' => $this->url->link('sale/sogantung/tampilcetak', 'token=' . $this->session->data['token'] . '&view=1&order_id=' . $result['sales_order_id'], 'SSL')
					);
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/sogantung', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();
		
		$this->data['cetakexcel'] = $this->url->link('sale/sogantung/exporttoexcel', 'token=' . $this->session->data['token'] . $url , 'SSL');
		$this->data['cetakbiasa'] = $this->url->link('sale/sogantung/cetakbiasa','token='.$this->session->data['token'].$url,'SSL');
		$this->data['filter_customer_id'] = $filter_customer_id;
		$this->data['filter_order_id']	= $filter_order_id;
		$this->data['filter_status']	= $filter_status;
		$this->data['filter_tanggal']	= $filter_tanggal;
