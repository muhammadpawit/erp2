<?php
class ControllerGudangProducthilang extends Controller {
	public function index() {
		$this->document->setTitle('Produk Hilang');
		...
		$this->data['insert'] = $this->url->link('gudang/producthilang/insert', 'token=' . $this->session->data['token'], 'SSL');
      ...
      'filter_gudang_id'			=> $filter_gudang_id,
		...
		$product_total = $this->model_gudang_product->getTotalProdukHilang($data,true);

		$results = $this->model_gudang_product->getProdukHilang($data,true);
		...
		$this->data['heading_title'] = 'Produk Hilang';
		...
		$pagination->url = $this->url->link('gudang/producthilang', 'token=' . $this->session->data['token'] . $url . '&page={page}');
		...
		$this->template = 'gudang/produkhilang.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_gudang_product->addProductHilang($this->request->post);

			$this->session->data['success'] = 'Sukses: Data produk hilang berhasil disimpan.';
			...
			$this->redirect($this->url->link('gudang/producthilang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['cancel']= $this->url->link('gudang/producthilang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('gudang/producthilang/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'gudang/producthilang_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'gudang/producthilang')) {
