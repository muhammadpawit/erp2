<?php
class ControllerLaporanDepositcustomer extends Controller {
	private $error = array();
	...
	private function getList() {
		...
		$this->data['totaldeposit']=$totaldeposit;
		$this->data['totalgiro']=$totalgiro;
		$this->data['totalpiutang']=$totalpiutang;
		$this->data['totalsisaharusbayar']=$totalsisaharusbayar;
		// end baru
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Deposit',
				'href' => $this->url->link('laporan/depositcustomer/deposit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL')
			);
			$action[] = array(
				'text' => 'History Giro',
				'href' => $this->url->link('laporan/depositcustomer/historygiro', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL')
			);
			$action[] = array(
				'text' => 'History Giro Belum Cair',
				'href' => $this->url->link('laporan/depositcustomer/historygirobelumcair', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL')
			);
			$title=null;
			$area=null;
			$this->data['customers'][] = array(
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('laporan/depositcustomer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		$this->data['exportexcel']= $this->url->link('laporan/depositcustomer', 'token=' . $this->session->data['token'] . $url . '&excel=1', 'SSL');
		...
		if(isset($this->request->get['excel'])){
			$this->template = 'laporan/depositcustomerexcel.tpl';	
		}else{
			$this->template = 'laporan/depositcustomer.tpl';
		}
		$this->children = array(
			'common/header',
			'common/footer'
