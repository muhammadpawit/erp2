<?php
class ControllerLaporanPemeliharaanaset extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/category');

		$this->document->setTitle('Laporan Pemeliharaan Aset');
	...
	private function getList() {
		...
		$product_total = count($this->model_catalog_aset->totalgets($data));
		//print_r($results);
		...
		$pagination->url = $this->url->link('laporan/pemeliharaanaset', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'laporan/pemeliharaanaset_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function autocompletejenispemeliharaan() {
		$json = array();
	...
	public function autocompletes() {
		...
		//if (isset($this->request->get['filter_name']) ) {
			...
			$data = array(
				...
				'pemeliharaan_aset.status'	=> array('<',3)
			);
