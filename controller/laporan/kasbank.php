<?php
class ControllerLaporanKasBank extends Controller {
	private $error = array();
	// baru 7 Januari 2019
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Mutasi',
				'href' => $this->url->link('laporan/kasbank/mutasi', 'token=' . $this->session->data['token'] . '&bank_id=' . $result['id'], 'SSL')
			);
		...
		$pagination->url = $this->url->link('laporan/kasbank/', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'laporan/kas_bank.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function mutasi() {
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('laporan/kasbank/mutasi', 'token=' . $this->session->data['token'] . $url . '&pagekartu={page}', 'SSL');
		...
		$this->template = 'laporan/mutasi.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
