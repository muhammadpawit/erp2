<?php
class ControllerLaporanAginghutang extends Controller {
	private $error = array();
	...
	private function getList() {
		...
		$this->data['cetak'] = $this->url->link('laporan/aginghutang', 'token=' . $this->session->data['token'] .'&cetak=true'. $url, 'SSL');
		...
		$results=$this->model_report_agingreport->aginghutang($data);
		$totaltagihan=0;
		$totaldeposit=0;
		$total=0;
		...
		if(!isset($this->request->get['cetak'])){
			$this->template = 'report/agingreporthutang_list.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
		...
		/*else if(!isset($this->request->get['excel'])){
			...
			$this->template = 'report/agingreporthutang_cetakexcel.tpl';
		}
