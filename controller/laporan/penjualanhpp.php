<?php
class ControllerLaporanPenjualanhpp extends Controller {
	private $error = array();
  ...
  /*      if (isset($this->request->get['filter_qty'])) {
   	...
   	if (isset($this->session->data['success'])) {
		...
		$this->load->model('gudang/product');
		$this->load->model('report/penjualan');
		...
		$this->data['penjualangudang']=$this->model_report_penjualan->getProductTerjualByGudang($data);
		$order_total = $this->model_report_penjualan->totalProductTerjuals($data);
		$results = $this->model_report_penjualan->getProductTerjuals($data);
		//print_r($results);exit;
		$this->data['products']=array();
		foreach ($results as $result) {
			//no_so
			$listso=$this->model_report_penjualan->listSo($result['product_id'],$result['gudang_id'],$data);
			//print_r($listso);
			$this->data['products'][] = array(
		...
		$pagination->url = $this->url->link('laporan/penjualanhpp', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'laporan/penjualanhpp.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  ...
  /*      if (isset($this->request->get['filter_qty'])) {
   	...
   	if (isset($this->session->data['success'])) {
		...
		$this->template = 'laporan/penjualanhppexcel.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
