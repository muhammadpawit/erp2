<?php
class ControllerLaporanfollowup extends Controller {
	private $error=array();
	...
	public function index() {
		...
		$product_total = count($this->model_followup_penagihan->totalgetlaporandata($filter));
		$results = $this->model_followup_penagihan->getlaporandata($filter);
		//$status=null;
		$cekinvoice=null;
		$cekdeposit=0;
		foreach ($results as $result) {
			...
			$cekinvoice=$this->model_followup_penagihan->cekinvoice($result['customer_id']);
			$cekdeposit=$this->model_followup_penagihan->cekdeposit($result['customer_id']);
			//if($custdata==1){
				if(!empty($cekinvoice)){
					//if($cekdeposit<=$cekinvoice){
					if(($cekdeposit-$cekinvoice) < 0){
						$action[] = array(
							'text' => 'Aging Report',
							'href' => $this->url->link('laporan/agingreport', 'token=' . $this->session->data['token'] . '&filter_type=2&filter_customer_id=' . $result['customer_id'].$url, 'SSL')
						);
					}
				}
			//}
			...
			$this->data['permintaans'][] = array(
				'id'	=> $result['id'],
				'customer_id' => $this->url->link('laporan/followup/history', 'token=' . $this->session->data['token'] . '&filter_customer_id=' . $result['customer_id'].$url, 'SSL'),
	...
	//	$this->data['gudangasals']=$this->model_catalog_gudang->getGudangs(true);
		...
		$this->template = 'followup/laporanfollowup.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
