<?php
class ControllerLaporanRekapMutasiStok extends Controller {
	private $error = array();
	// baru 4 Maret 2020
	...
	function xlscreation_direct() {
		...
		$fileName = "Laporan_Rekap_Mutasi_Stok_".$presentDate.".xlsx";
	...
	public function cetakexcel() {
		...
		$this->data['cetak'] = $this->url->link('laporan/rekapmutasistok/cetak', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['uploadex'] = $this->url->link('laporan/rekapmutasistok/uploadexcel', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$action = array();
			$quantityawal=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
			$stokmasuk=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
			$stokkeluar=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
			$saldo=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
      		$this->data['products'][] = array(
				'gudang_id'   => $result['gudang_id'],
				'Produk ID'   => $result['product_id'],
				'Nama Produk'   => $result['name'],
				'Gudang'   => $result['nama'],
				'Quantityawal'   => !empty($quantityawal)?$quantityawal['quantityawal']:0,
				'Stokmasuk'  => !empty($stokmasuk)?$stokmasuk['stokmasuk']:0,
				'Stokkeluar'  => !empty($stokkeluar)?$stokkeluar['stokkeluar']:0,
				'Saldo'  => !empty($saldo)?$saldo['saldo']:0,
	...
	public function index() {
		...
		$alls = $this->model_gudang_product->getProducts($all,true);
		$awal=0;
		$masuk=0;
		$keluar=0;
		$totalsaldo=0;
		foreach($alls as $result){
			...
			$stokkeluar=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
			$saldo=$this->model_gudang_product->getqty($result['product_id'],$result['gudang_id'],$data);
			$awal += !empty($quantityawal)?$quantityawal['quantityawal']:0;
			$masuk += !empty($stokmasuk)?$stokmasuk['stokmasuk']:0;
			$keluar+= !empty($stokkeluar)?$stokkeluar['stokkeluar']:0;
			$totalsaldo+= !empty($saldo)?$saldo['saldo']:0;

		}

		$this->data['totalawal']=$awal;
		$this->data['totalmasuk']=$masuk;
		$this->data['totalkeluar']=$keluar;
		$this->data['totalsaldo']=$totalsaldo;
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('laporan/rekapmutasistok', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->data['cetak'] =$this->url->link('laporan/rekapmutasistok/exporttoexcel', 'token=' . $this->session->data['token'] . $url , 'SSL');

		$this->template = 'report/rekapmutasistok.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
