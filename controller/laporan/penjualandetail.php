<?php
class ControllerLaporanPenjualandetail extends Controller {
	private $error = array();
	...
	private function getList() {
		...
		if (isset($this->request->get['jenistgl'])) {
			$jenistgl = $this->request->get['jenistgl'];
		} else {
			$jenistgl =1;
		}
		...
		if($jenistgl==1){
			...
			$jumlah=$this->model_sale_invoice->getlapsalebysj($filter);
			if($this->user->getUsername()=="pawitx"){
				echo "<pre>";print_r($jumlah);exit;
			}
			if($filter_sales==null){
				$this->data['jumlah']=$jumlah['total'];
				$this->data['jumlahtanpapajak']=$jumlah['total']-$jumlah['pajak'];
			}
		}
		
		
		/*
		$p = $this->model_sale_invoice->getlapsalebysj($filter);
		foreach($p as $jumlah){
			$this->data['jumlah']+=round($jumlah['total']);
			$this->data['jumlahtanpapajak']+=round($jumlah['total']-$jumlah['pajak']);
		}
		$this->data['jt']=count($p);*/
		//$this->data['jumlahtanpapajak']=$this->currency->format($jumlah['total']-$jumlah['totalpajak']);
		...
		/*}else{
			$results = $this->model_sale_invoice->getlapsalebysj($filter);
		}*/
		...
		foreach ($results as $result) {
			$d = $this->model_sale_invoice->getnoso($result['id']);
			$sales=$this->model_user_user->getUser($d['sales']);
			$tglsj=$this->model_sale_invoice->gettglsj($result['id']);
			$nosj=$this->model_sale_invoice->getnosj($result['id']);
			$action = array();
			...
			if($filter_sales!=null){
				if($filter_sales==$d['sales']){
					...
					$this->data['penjualans'][] = array(
						...
						'pajak'	=> $this->currency->format($result['pajak']),
						'status'	=> $result['status'],
						'tglsj'=>($tglsj==null)?'':date('d/m/y',strtotime($tglsj)),
						'nosj'=>($nosj==null)?'':$nosj,
						'tanggal'	=>date('d/m/y',strtotime($result['date_added'])),
						'register'	=>date('d/m/y',strtotime($result['register'])),
		...
		$pagination->url = $this->url->link('laporan/penjualandetail', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->data['exporttoexcel'] = $this->url->link('laporan/penjualandetail/exporttoexcel', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['filter_provinsi'] = $filter_provinsi;
		$this->data['sort'] = $sort;
		$this->data['order'] = $order;

		if(!isset($this->request->get['print'])){
			$this->template = 'laporan/penjualandetail_list.tpl';
			$this->children = array(
				'common/header',
				'common/footer'
			);
