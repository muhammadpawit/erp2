<?php
class ControllerLaporanMutasipiutangdetail extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->setTitle('Laporan Mutasi Piutang Detail');
	...
	private function getList() {
        ...
        if(!is_null($filter_date_start)){
            ...
            //get status
            $this->load->model('report/laporanmutasidetail');
            $nominal=0;
            $piutang=0;
            ...
            //	$nominal = $this->model_sale_customer->getnominalgiro($result['customer_id']);
                ...
                $title=null;
                $area=null;
                $saldoawal=$this->model_report_laporanmutasidetail->getSaldoAwal(array('tanggal'=>$filter_date_start,'customer_id'=>$result['customer_id']));
                $penambahan=$this->model_report_laporanmutasidetail->penambahan(array('filter_date_start'=>$filter_date_start,'filter_date_end'=>$filter_date_end,'customer_id'=>$result['customer_id']));
                $pelunasan= $this->model_report_laporanmutasidetail->pembayaran(array('filter_date_start'=>$filter_date_start,'filter_date_end'=>$filter_date_end,'customer_id'=>$result['customer_id']));
                $returjual=$this->model_report_laporanmutasidetail->returnpenjualan(array('filter_date_start'=>$filter_date_start,'filter_date_end'=>$filter_date_end,'customer_id'=>$result['customer_id']));
                $uangmuka=$this->model_report_laporanmutasidetail->uangmuka(array('filter_date_start'=>$filter_date_start,'filter_date_end'=>$filter_date_end,'customer_id'=>$result['customer_id']));
				$penyesuaian=$this->model_report_laporanmutasidetail->penyesuaian(array('filter_date_start'=>$filter_date_start,'filter_date_end'=>$filter_date_end,'customer_id'=>$result['customer_id']));
				//returjual
                //uang muka
                //adjusment
                //saldo akhir
                $this->data['customers'][] = array(
                    ...
                    'returjual'   => $this->currency->format($returjual),
					'uangmuka'   => $this->currency->format($uangmuka-$penyesuaian),
					'penyesuaian'   => $this->currency->format($penyesuaian),
					'saldoakhir'	=> $this->currency->format($saldoawal+$penambahan-$pelunasan-$returjual-$uangmuka),
		...
		$pagination->url = $this->url->link('laporan/mutasipiutangdetail', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'laporan/mutasipiutangdetail.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
