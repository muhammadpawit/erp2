<?php
class ControllerKeuanganBankCabang extends Controller {
	private $error = array();
	// baru 20 Januari 2020 - Function mutasi
	public function mutasi() {

		$this->document->setTitle('Mutasi Bank Kas Cab.Surabaya');
		...
		$this->data['url'] = $this->url->link('keuangan/bankcabang/mutasi', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->get['bank_id'])) {
			...
			$this->redirect($this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->load->model('keuangan/bankcabang');
		$data = array(
		...
		$results=$this->model_keuangan_bankcabang->laporanmutasibank($bank_id,$data);
		$order_total=$this->model_keuangan_bankcabang->totallaporanmutasibank($bank_id,$data);;
		$this->data['totalsaldo']=0;
		$this->data['saldoawal']=$this->model_keuangan_bankcabang->saldoawalnew($bank_id);
		$this->data['bank']=$this->model_keuangan_bankcabang->getBank(array(),array(),array('id'  => $bank_id));
		$this->data['cancel'] = $this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['exportexcel'] = $this->url->link('keuangan/bankcabang/exportexcel', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$tot = $this->model_keuangan_bankcabang->totalan($data);
		...
		foreach (array_reverse($results) as $result) {
			...
			if($custdata){
				...
				$action[] = array(
					'text' => 'Edit Mutasi',
					'href' => $this->url->link('keuangan/bankcabang/editmutasi', 'token=' . $this->session->data['token'] . '&bank_id='.$bank_id.'&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Hapus Mutasi',
					'href' => $this->url->link('keuangan/bankcabang/hapusmutasi', 'token=' . $this->session->data['token'] . '&bank_id='.$bank_id.'&id=' . $result['id'].$url, 'SSL')
				);
		...
		$pagination->url = $this->url->link('keuangan/bankcabang/mutasi', 'token=' . $this->session->data['token'] . $url . '&pagekartu={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['jeniss']=$this->model_keuangan_bankcabang->getTypeMutasi();
	...
	public function exportexcel() {
		...
		$this->data['url'] = $this->url->link('laporan/kasbank/mutasi', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->data['saldoawal']=$this->model_keuangan_bankcabang->saldoawalnew($bank_id);
		$this->data['bank']=$this->model_keuangan_bankcabang->getBank(array(),array(),array('id'  => $bank_id));
		$this->data['cancel'] = $this->url->link('laporan/kasbank', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['exportexcel'] = $this->url->link('laporan/kasbank/exportexcel', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$tot = $this->model_keuangan_bankcabang->totalan($data);
		$this->data['totalsaldomasuk'] = $tot['saldomasuk'];
		$this->data['totalsaldokeluar'] = $tot['saldokeluar'];
		if($this->user->getUsername()=="pawits"){
		...
		foreach (array_reverse($results) as $result) {
			...
			if($custdata){
				...
				$action[] = array(
					'text' => 'Edit Mutasi',
					'href' => $this->url->link('laporan/kasbank/editmutasi', 'token=' . $this->session->data['token'] . '&bank_id='.$bank_id.'&id=' . $result['id'].$url, 'SSL')
				);
				$action[] = array(
					'text' => 'Hapus Mutasi',
					'href' => $this->url->link('laporan/kasbank/hapusmutasi', 'token=' . $this->session->data['token'] . '&bank_id='.$bank_id.'&id=' . $result['id'].$url, 'SSL')
				);
			...
			if($a==0){
				...
				//echo $result['saldomasuk'];
				//echo $sisa;
				$sisa += ($result['saldomasuk'] - $result['saldokeluar']);
			}
			//echo $a;
			//print_r($result['tanggal_transaksi']);
			//exit;
			if($filter_tgl_awal==''){
				if($result['tanggal_transaksi'] < date('Y-m-d',strtotime('first day of this month'))){
					$sblm = $sisa;
				}
				if($result['tanggal_transaksi'] >= date('Y-m-d',strtotime('first day of this month')) && $result['tanggal_transaksi'] <= date('Y-m-d',strtotime('last day of this month'))){
					$totalmasuk += $result['saldomasuk'];
					$totalkeluar += $result['saldokeluar'];
					$this->data['kartustoks'][] = array(
						'id'	=> $result['id'],
				...
				if($result['tanggal_transaksi'] >= $filter_tgl_awal && $result['tanggal_transaksi'] <= $filter_tgl_akhir){
					$totalmasuk += $result['saldomasuk'];
					$totalkeluar += $result['saldokeluar'];
					$this->data['kartustoks'][] = array(
						'id'	=> $result['id'],
	...
	public function jurnal(){
		...
		$sql="SELECT * FROM jurnal_umum WHERE type=1 and ref='".$id."' ";
		$j = $this->db->query($sql);
		$jd = $this->db->query("SELECT * FROM jurnal_umum_detail WHERE jurnal_id='".$j->row['id']."' order by urutan asc");
		$linkterkait = ($j->row['linkterkait']==null)?$j->row['ref']:$j->row['ref'].' '.$j->row['linkterkait'];
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_keuangan_bankcabang->addBank($this->request->post);
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$option_info = $this->model_keuangan_bankcabang->getBank(array(),array(),array('id'	=> $this->request->get['bank_id']),array());
			...
			$this->model_keuangan_bankcabang->editBank($data,array('id'	=> $this->request->get['bank_id']));
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $bank_id) {
				$this->model_keuangan_bankcabang->updateBank(array('hapus'=>1),array('id'	=> $bank_id));
			}
	...
	private function getList() {
		...
		$this->data['insert'] = $this->url->link('keuangan/bankcabang/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('keuangan/bankcabang/delete', 'token=' . $this->session->data['token'], 'SSL');
		...
		$results = $this->model_keuangan_bankcabang->getBanks(array(),array(),$data,array('currency' => 'ASC','name'	=> 'ASC'),$limit,$offset);
		$product_total = $this->model_keuangan_bankcabang->getBank(array('count(*) as total'),array(),$data,array(),0,null);
		$i=0;
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => 'Edit',
				'href' => $this->url->link('keuangan/bankcabang/update', 'token=' . $this->session->data['token'] . '&bank_id=' . $result['id'], 'SSL')
			);
			$action[] = array(
				'text' => $this->language->get('Input Transaksi'),
				'href' => $this->url->link('keuangan/bankcabang/inserttransaksi', 'token=' . $this->session->data['token'] . '&bank_id=' . $result['id'] . $url, 'SSL')
			);
			$action[] = array(
				'text' => 'Mutasi',
				'href' => $this->url->link('keuangan/bankcabang/mutasi', 'token=' . $this->session->data['token'] . '&bank_id=' . $result['id'], 'SSL')
			);
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'bankcabang/bank_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() {
		...
		if (!isset($this->request->get['bank_id'])) {
			$this->data['action'] = $this->url->link('keuangan/bankcabang/insert', 'token=' . $this->session->data['token'].$url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('keuangan/bankcabang/update', 'token=' . $this->session->data['token'].$url. '&bank_id=' . $this->request->get['bank_id'], 'SSL');
		}

		$this->data['cancel'] = $this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->template = 'bankcabang/bank_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function mutasi() {
		...
		$order_total = $this->model_keuangan_bankcabang->getTotalRecordSaldo($data);

		$results = $this->model_keuangan_bankcabang->getSaldo($data);
		$this->data['totalsaldo']=$this->model_keuangan_bankcabang->getSelectedSaldo($data);
		$this->data['bank']=$this->model_keuangan_bankcabang->getBank(array(),array(),array('id'  => $bank_id));
		...
		$i=0;
		foreach ($results as $result) {
			$saldosebelumnya=$this->model_keuangan_bankcabang->getsaldosebelumnya($result['bank_id']);
			if($this->user->getUsername()=="pawits"){
	...
	*/
	...
	public function mutasi() {
		...
		//$order_total = $this->model_keuangan_bankcabang->getTotalRecordSaldo($data);
		//$results = $this->model_keuangan_bankcabang->getSaldo($data);
		$results = $this->model_keuangan_bankcabang->getsaldosebelumnya($bank_id,$data);
		$order_total = count($this->model_keuangan_bankcabang->totalgetsaldosebelumnya($bank_id,$data));
		$this->data['totalsaldo']=$this->model_keuangan_bankcabang->getSelectedSaldo($data);
		$this->data['saldoawal']=$this->model_keuangan_bankcabang->getSelectedSaldoAwal($data);
		$this->data['bank']=$this->model_keuangan_bankcabang->getBank(array(),array(),array('id'  => $bank_id));
		$this->data['cancel'] = $this->url->link('keuangan/bank', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$saldosebelumnya=$this->model_keuangan_bankcabang->getsaldosebelumnya($result['bank_id'],$data);
	...
	*/
	...
	public function inserttransaksi() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormTransaksi()) {
			...
			$this->model_keuangan_bankcabang->addTransaksi($this->request->post);
			...
			$this->redirect($this->url->link('keuangan/bankcabang/mutasi', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function getFormTransaksi() {
		...
		if (!isset($this->request->get['bank_id'])) {
			$this->redirect($this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] , 'SSL'));
		} else {
			if(empty($this->request->get['bank_id'])){
				$this->redirect($this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] , 'SSL'));
			}else{
				$this->data['action'] = $this->url->link('keuangan/bankcabang/inserttransaksi', 'token=' . $this->session->data['token'] . '&bank_id=' . $this->request->get['bank_id'] . $url, 'SSL');
			}
		}

		$this->data['cancel'] = $this->url->link('keuangan/bankcabang', 'token=' . $this->session->data['token'] , 'SSL');
		$this->data['banks']	= $this->model_keuangan_bankcabang->getBanks();
		$this->data['token'] = $this->session->data['token'];

		$this->template = 'bankcabang/input_transaksi.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function autocomplete(){
		...
		$this->load->model('keuangan/bankcabang');
			...
			$results = $this->model_keuangan_bankcabang->getBanks($column,$join,$data,array(),$limit,$start);
	...
	if (isset($this->request->get['filter_jenis'])) {
		...
		if (isset($this->request->get['id'])) {
			...
			if(empty($this->request->get['id'])){
				...
				$this->data['action'] = $this->url->link('keuangan/bankcabang/editmutasi', 'token=' . $this->session->data['token'] . '&bank_id=' . $this->request->get['bank_id'] . $url, 'SSL');
			}
		}

		$mutasi=$this->model_keuangan_bankcabang->getDetailSaldo($id);
		$this->data['mutasi']=$mutasi;

		$bank=$this->model_keuangan_bankcabang->getBank(array(),array(),array('id'=>$mutasi['bank_id']));
		$this->data['bank']=$bank;
		$this->document->setTitle("Edit Mutasi ".$bank['name']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
			$this->model_keuangan_bankcabang->editTransaksi($this->request->post,$this->request->get['id']);
		...
		$this->template = 'bankcabang/editmutasi.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
		...
		if (isset($this->request->get['filter_jenis'])) {
			...
			$this->model_keuangan_bankcabang->hapusTransaksi(array('id'=>$this->request->get['id']));
