...
class ControllerKeuanganCoa extends Controller {
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			if($this->user->getUsername()=="pawixt"){
				echo "<pre>";print_r($this->request->get['category_id']);exit;
			}
	...
	private function getList() {
		...
		$this->data['saldoawal'] = $this->url->link('keuangan/coa/saldoawal', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		//$this->data['totaldebet']=$this->currency->format($this->model_keuangan_jurnal->totalDebet(0));
		//$this->data['totalkredit']=$this->currency->format($this->model_keuangan_jurnal->totalKredit(0));

		//echo count($results);
		foreach ($results as $result) {
			//$debet=$this->model_keuangan_jurnal->totalDebet($result['kode_rek']);
			//$kredit=$this->model_keuangan_jurnal->totalKredit($result['kode_rek']);
			$action = array();
	...
	public function saldoawal() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormSaldo()) {
      $no_po=$this->model_keuangan_coa->addSaldo($this->request->post);

			$this->session->data['success'] = 'Sukses: Saldo Awal  berhasil disimpan';
		...
		$this->data['cancel']= $this->url->link('keuangan/coa', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['action']= $this->url->link('keuangan/coa/saldoawal', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'keuangan/saldoawal_akun.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateFormSaldo() {


		//cek status tutup bulan
		//print_r($cek);
		if (!$this->error) {
	  		return true;
		} else {
